...
On the web, a raw HTML `id` is not treated as a stable developer identifier.
Frameworks and applications commonly allocate it at render time, and one DOM
capture cannot determine its lifetime. Web identity and generated replay
selectors therefore use explicit author contracts (`data-testid`,
`data-test-id`, or `name`) and otherwise use role plus structural index. Other
platforms retain their documented stable platform identifiers.
...
(metadata only; it doesn't affect the hash), so `debug map show` can warn you to add
one.
...
(`debug map show` explains why two screens are the same or different, and config can
