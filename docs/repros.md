# What a repro is made of, and what it proves

A causal capsule is the executable artifact behind a confirmed finding: the minimized action path
plus only the external inputs needed to reproduce that exact failure. This page is the contract it
holds itself to, so you can tell a real reproduction from a coincidence.

You never manage capsules directly. They are produced and consumed by the ordinary commands:
...
Which platforms can record a dependency boundary, and how each is proven. `captured` means the adapter can
