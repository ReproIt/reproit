...
export function installCausalFetch(excludePrefix?: string | null): () => void {
  ...
  try {
    if (capsulePath) capsule = JSON.parse(fs.readFileSync(capsulePath, 'utf8'));
  } catch {}
  ...
  const actionIndex = () => {
    try {
      return Number(fs.readFileSync(actionFile, 'utf8')) || 0;
    } catch {
      return 0;
    }
  };
  ...
  (globalThis as any).fetch = async (input: any, init: AnyMap = {}) => {
    ...
    if (capsule) {
      const idx = exchanges.findIndex(
        (e, i) =>
          !used.has(i) &&
          e.required &&
          e.actor === (proc.env.REPROIT_DEVICE || 'a') &&
          e.actionIndex === action &&
          String(e.method).toUpperCase() === method &&
          canonical(String(e.url)) === canonical(url),
      );
    ...
    new Headers(init.headers || {}).forEach((v, k) => {
      headers[k] = SECRET.test(k) ? '<reproit:secret>' : v;
    });
    const responseHeaders: AnyMap = {};
    response.headers.forEach((v: string, k: string) => {
      responseHeaders[k] = SECRET.test(k) ? '<reproit:secret>' : v;
    });
    const actor = proc.env.REPROIT_DEVICE || 'a';
    const exchange = {
      id: `${actor}-${action}-${thisOrdinal}`,
      actor,
      actionIndex: action,
      ordinal: thisOrdinal,
    ...
    try {
      fs.appendFileSync(networkFile, JSON.stringify(exchange) + '\n', { mode: 0o600 });
    } catch {}
    return response;
  };
  return () => {
