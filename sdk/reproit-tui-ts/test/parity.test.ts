...
test("value_class buckets and bounded numeric extraction", () => {
  ...
  // bounded: 50 numbers cap at MAX_VALUE_CLASSES (8).
  let many = "";
  for (let n = 0; n < 50; n++) many += n + " ";
  assert.equal(numericValueClasses(many).length, 8, "value-class set is capped");
...
test("flush POSTs the {appId, sentAt, ctx?, events} batch", () => {
  ...
  assert.ok(posted, "flush POSTed a batch");
  const batch = JSON.parse(posted!.body);
