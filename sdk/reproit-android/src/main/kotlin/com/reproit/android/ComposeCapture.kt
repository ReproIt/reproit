...
/**
 * Android-side bridge from a hosted Jetpack Compose view to the pure-Kotlin
 * [Compose] mapper.
 *
 * A `ComposeView` renders into an `AndroidComposeView` (the concrete
 * [androidx.compose.ui.node.RootForTest]) that exposes the `SemanticsOwner`, the
 * SAME semantics tree TalkBack and the Appium/UiAutomator2 runner read. We walk
 * that tree, read each node's `SemanticsProperties.*` reflection-free, and build a
 * framework-free [Compose.ComposeSemantics] holder; [Compose.toNodes] then folds
 * those into canonical [Signature.Node]s so the unchanged [Signature] core hashes
 * them identically to what the runner sees.
 *
 * This file imports `androidx.compose.ui.*`, so (like [ReproIt]) it needs the
 * Compose dependency and is EXCLUDED from the host parity test. The Compose
 * dependency is `compileOnly`: an app that does not use Compose simply has no
 * `RootForTest` on screen, so [extractRootForTest] returns null and nothing here
 * runs. The pure structural mapping it delegates to ([Compose]) IS host-tested.
 */
internal object ComposeCapture {

    /**
     * If [view] is a hosted Compose root (an `AndroidComposeView`, which implements
     * [RootForTest]), return the canonical [Signature.Node] children produced by
     * walking its semantics tree, to be spliced in place of the opaque view's
     * children. Returns null when [view] is not a Compose root (so the caller
     * keeps recursing the ordinary View tree). Any failure degrades to null so a
    ...
    /** Detect the Compose root that exposes the semantics owner. The concrete
     * `AndroidComposeView` implements [RootForTest]; we match by interface so we do
     * not depend on the internal class name. */
    private fun extractRootForTest(view: View): RootForTest? =
        view as? RootForTest

    /**
     * Read one [SemanticsNode] into a framework-free [Compose.ComposeSemantics],
     * recursively. All reads are reflection-free through the public
     * `SemanticsProperties` / `SemanticsActions` keys, so they are stable across
     * Compose versions that keep those public keys.
     */
    private fun read(node: SemanticsNode): Compose.ComposeSemantics {
        val cfg = node.config

        val role: String? = cfg.getOrNull(SemanticsProperties.Role)
            ?.toString()
            ?.lowercase()
        ...
        val isSlider = range != null &&
            cfg.getOrNull(SemanticsActions.SetProgress) != null
        ...
        val liveText: String? = if (liveRegion) {
    ...
    /** Render a slider/progress value locale-independently: a whole number with no
     * trailing `.0` (so `5f` -> "5" -> POS1 through the strict-decimal grammar), a
     * fractional one with its period decimal. */
