...
/** Reduced snapshot: a signature, capped unique labels, and unlabeled-tappable count. */
data class Snapshot(
    val sig: String,
...
data class Step(val sig: String, val action: String) {
    fun toMap(): Map<String, Any?> = mapOf("sig" to sig, "action" to action)
}
...
class Engine(
     ...
     * (`map --show`): it dedupes labels, caps to [ReproItConfig.maxLabels], and
     * counts clickable-but-unnamed nodes as `unlabeled`. Labels do NOT affect the
     * signature.
     */
    fun reduce(
        nodes: List<RawNode>,
        ...
        for (n in nodes) {
            ...
            if (label == null) {
                if (n.tappable && n.name.trim().isEmpty()) unlabeled++
                continue
            }
            seen.add(label)
        }
        ...
        return Snapshot(sig, unique.take(cfg.maxLabels), unlabeled)
    }

    /** Record the action a tap implies; consumed by the next state change. */
    fun noteTap(label: String?) {
        pendingAction = if (label != null && label.isNotEmpty()) "tap:$label" else "tap:?"
    }
    ...
    fun observe(snap: Snapshot, firstAction: String = "load") {
        ...
        val action = pendingAction ?: "auto"
        pendingAction = null
        emitEdge(from = cur, action = action, to = snap, append = true)
        currentSig = snap.sig
    }

    private fun emitEdge(from: String?, action: String, to: Snapshot, append: Boolean) {
        if (append) {
            path.add(Step(from ?: "", action))
