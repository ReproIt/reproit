...
class CausalHttp {
    ...
    private val capsulePath = (System.getenv("REPROIT_CAPSULE") ?: systemProperty("debug.reproit.capsule"))
        ?.takeUnless { it == "__reproit_none__" }
    private val active = networkFile != null || capsulePath != null ||
        System.getenv("REPROIT_CAUSAL") == "1" || systemProperty("debug.reproit.fuzz") == "1"
    ...
    init {
        ...
        if (active) {
            android.util.Log.d("reproit", "REPROIT:CAPABILITIES {\"http\":{\"status\":\"captured\"},\"http_replay\":{\"status\":\"captured\"}}")
        }
    }
    ...
    fun request(
        ...
        val currentOrdinal = synchronized(lock) {
            if (previousAction != action) { previousAction = action; ordinal = 0 }
            ordinal++
            ordinal - 1
        }
        if (capsulePath != null) {
            val match = synchronized(lock) {
                exchanges.withIndex().firstOrNull { (index, exchange) ->
                    index !in used && exchange["required"] == true &&
                        exchange["actor"] == actor && number(exchange, "actionIndex", "action_index") == action &&
                        exchange["method"].toString().equals(method, true) &&
                        canonical(exchange["url"].toString()) == canonical(url)
                }?.also { used += it.index }?.value
            } ?: throw IllegalStateException("CAPSULE:MISS ${method.uppercase(Locale.ROOT)} $url action=$action")
            val responseBody = match["responseBody"] ?: match["response_body"]
            val bytes = if (responseBody is String) responseBody.toByteArray() else Json.encode(responseBody).toByteArray()
            @Suppress("UNCHECKED_CAST")
            val responseHeaders = (match["responseHeaders"] ?: match["response_headers"] ?: emptyMap<String, String>()) as Map<String, String>
        ...
        val exchange = linkedMapOf<String, Any?>(
            "id" to "$actor-$action-$currentOrdinal", "actor" to actor,
            "actionIndex" to action, "ordinal" to currentOrdinal,
            "protocol" to URI(url).scheme, "method" to method.uppercase(Locale.ROOT), "url" to url,
            "requestHeaders" to redactHeaders(headers), "requestBody" to bodyValue(body, headers),
            "status" to response.status, "responseHeaders" to redactHeaders(response.headers),
            "responseBody" to bodyValue(response.body, response.headers), "required" to true,
        )
        val marker = "REPROIT:EXCHANGE ${Json.encode(exchange)}"
        android.util.Log.d("reproit", marker)
        try { networkFile?.let { File(it).appendText(Json.encode(exchange) + "\n") } } catch (_: Throwable) {}
        return response
    }

    private fun live(url: String, method: String, headers: Map<String, String>, body: ByteArray?, connectMs: Int, readMs: Int): Response {
        ...
        try {
            ...
            val responseHeaders = connection.headerFields
                .filterKeys { it != null }.mapValues { (_, values) -> values.joinToString(",") }
    ...
    private fun actionIndex(): Int {
        systemProperty("debug.reproit.action")?.toIntOrNull()?.let { return it }
        return try { File(System.getenv("REPROIT_ACTION_FILE") ?: return 0).readText().trim().toInt() } catch (_: Throwable) { 0 }
    }

    private fun systemProperty(name: String): String? = try {
        val type = Class.forName("android.os.SystemProperties")
        (type.getMethod("get", String::class.java).invoke(null, name) as? String)?.takeIf { it.isNotBlank() }
    } catch (_: Throwable) { null }

    private fun canonical(raw: String): String = try {
        val uri = URI(raw)
        val query = uri.rawQuery?.split("&")?.sorted()?.joinToString("&")
        URI(uri.scheme?.lowercase(), uri.userInfo, uri.host?.lowercase(), uri.port, uri.path, query, uri.fragment).toString()
