...
/**
 * Minimal JSON encoder for the event payloads. Pure Kotlin (no `org.json`, which
 * is an Android stub on the host JVM and not available to plain `kotlinc`), so
 * the JSON-shape tests run on the host. Only encodes the value types the event
 * model uses: String, Int, Long, Double, Boolean, null, List, and Map.
 */
object Json {
    ...
    /**
     * Minimal recursive-descent JSON decoder. Pure Kotlin (no `org.json`, which
     * is an Android stub on the host JVM and unavailable to plain `kotlinc`), so
     * the parity test can parse `signature_vectors.json` on the host. Returns the
     * usual object graph: Map<String, Any?>, List<Any?>, String, Double, Boolean,
     * or null. Sufficient for the golden-vector schema; not a full validator.
     */
