 ...
 *
 * The native fuzzer drives the Android app and cannot call the app's predicates,
 * so the SDK evaluates its OWN registered invariants on each settled state and,
 * ONLY under the fuzzer, logs a `REPROIT_INVARIANT` marker (on logcat) that
 * runners/rn/runner.mjs scrapes into an EXPLORE:INVARIANT line. The registry +
 * evaluation + marker shape live in the pure-Kotlin [Engine] (this test), while
 * the fuzzer gate + `android.util.Log` emission live in the android-only
 * [ReproIt] layer (not host-testable, verified by the appium smoke). Both
 * directions are asserted: a VIOLATING invariant produces a marker, a CLEAN one
 * is silent.
 */
class InvariantTest {
    private fun engine(): Engine =
        Engine(cfg = ReproItConfig(appId = "t"), now = { 0L })
