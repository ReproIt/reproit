...
/**
 * Host-side unit tests for the pure-Kotlin Jetpack Compose semantics mapping ([Compose]). These
 * cover the structural mapping that turns a Compose semantics tree into the canonical
 * [Signature.Node] descriptor, WITHOUT the Android SDK or the Compose runtime (the androidx access
 * lives in `ComposeCapture.kt`, which is excluded from the host test exactly like `ReproIt.kt`).
 *
 * The contract under test: a Compose UI must fold into the SAME [Signature.Node] model the View
 * walk produces, so the unchanged [Signature] core hashes it byte-for-byte like the runner that
 * reads the same semantics tree.
 */
class ComposeMappingTest {
  ...
  fun composeTreeProducesSameDescriptorAsEquivalentViewTree() {
    ...
    val composeRoots =
      listOf(cs(role = "button", testTag = "submit", clickable = true, hasText = true))
    // The hand-built canonical equivalent the runner would produce.
    val expected = Signature.Node(role = "button", id = "submit")
    assertEquals(
  ...
  fun composeScreenMatchesRunnerStyleStructure() {
    ...
    assertEquals(Signature.descriptor("/login", expected), Signature.descriptor("/login", screen))
  }
  ...
  fun editableTextFieldFoldsValueClassIntoVSection() {
    ...
    assertEquals("A:\n0:textfield:text@email\nV:key:email=EMPTY", Signature.descriptor(null, empty))
    val filled = Compose.toNode(cs(editable = true, testTag = "email", editableValue = "a@b.com"))
    assertEquals(
  ...
  fun sliderValueIsFlaggedAndBucketed() {
    ...
    val node =
      Compose.toNode(cs(testTag = "vol", hasProgress = true, slider = true, rangeValue = "5"))
    assertEquals("slider", node.role)
    assertEquals(true, node.valueNode)
    assertEquals("A:\n0:slider@vol\nV:key:vol=POS1", Signature.descriptor(null, node))
    // 0 vs 5 are distinct buckets (ZERO vs POS1), matching the oracle.
    val at0 =
      Compose.toNode(cs(testTag = "vol", hasProgress = true, slider = true, rangeValue = "0"))
  ...
  fun readOnlyProgressIndicatorIsTransient() {
    ...
    assertEquals(Signature.descriptor(null, without), Signature.descriptor(null, screen))
  }

  @Test
  fun liveRegionStatusFoldsValueClass() {
    val node =
      Compose.toNode(cs(liveRegion = true, testTag = "count", hasText = true, liveText = "12"))
    // live region -> structural role `text` (has text), flagged value_node so
    // the status value enters V:.
    assertEquals("A:\n0:text@count\nV:key:count=POS2", Signature.descriptor(null, node))
  }
  ...
  fun repeatedComposeListItemsCollapse() {
    ...
    fun row() = cs(clickable = true, children = listOf(cs(hasText = true)))
