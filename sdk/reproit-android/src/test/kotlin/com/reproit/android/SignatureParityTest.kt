 ...
 *
 * It loads the canonical golden vectors at the repo root (`signature_vectors.json`) and asserts the
 * Kotlin implementation produces `expected_sig` for every vector, exactly as the Rust oracle's
 * `tests::golden_vectors_match` and the Flutter / web parity tests do. If a vector mismatches, the
 * failure prints the descriptor string so you can diff it against docs/signature.md before touching
 * anything. Never edit the vectors or the oracle to make this pass.
 *
 * The descriptor that gets hashed is byte-identical to the Rust oracle: token =
 * <depth>:<role>[:<type>][#<icon>][@<id>] (trailing `*` on a repeat) desc = "A:" + anchor + "\n" +
 * tokens.join(";") sig = FNV-1a 32-bit over UTF-8(desc), 8-char lowercase hex.
 *
 * This test imports only pure-Kotlin classes ([Signature], [Engine], [Json], [Fingerprint]) and has
 * NO `android.*` dependency, so it runs on the host JVM without the Android SDK.
 */
class SignatureParityTest {
  ...
  private fun loadVectors(): List<Vector> {
    ...
    @Suppress("UNCHECKED_CAST") val list = Json.decode(raw) as List<Map<String, Any?>>
    return list.map { j ->
  ...
  fun valueStateDescriptorShapeMatchesSpec() {
    ...
    assertEquals(
      "A:\n0:header@title",
      Signature.descriptor(null, Signature.Node(role = "header", id = "title", value = "Welcome")),
    )
    ...
    assertEquals(
      "A:\n0:node@count\nV:key:count=POS1",
      Signature.descriptor(null, Signature.Node(role = "status", id = "count", value = "5")),
    )
    ...
    assertEquals(
      "A:\n0:text@display",
      Signature.descriptor(null, Signature.Node(role = "text", id = "display", value = "42")),
    )
    assertEquals(
      "A:\n0:text@display\nV:key:display=POS2",
      Signature.descriptor(
        ...
        children =
          listOf(
            Signature.Node(
              role = "group",
              transient = true,
              children = listOf(Signature.Node(role = "status", id = "loading", value = "50")),
            )
          ),
      )
  ...
  fun descriptorShapeMatchesSpec() {
    ...
    val withoutSpinner =
      Signature.Node(role = "screen", children = listOf(Signature.Node(role = "text")))
    assertEquals(
  ...
  fun signatureExcludesLocalizedText() {
    ...
    val en =
      engine.reduce(listOf(RawNode("Welcome", false), RawNode("Continue", false)), tree, "/login")
  ...
  fun edgeAndErrorPayloadsMatchContract() {
    ...
    engine.observe(
      engine.reduce(
        listOf(RawNode("Settings", false), RawNode("Back", false)),
        settings,
        "/settings",
      )
    )
    ...
    @Suppress("UNCHECKED_CAST") val labels = tap["labels"] as List<String>
    ...
    @Suppress("UNCHECKED_CAST") val path = err["path"] as List<Map<String, Any?>>
    assertEquals(2, path.size)

    val capture = requireNotNull(engine.captureBug())
    assertEquals("tester-capture", capture["oracle"])
    @Suppress("UNCHECKED_CAST") val identity = capture["findingIdentity"] as Map<String, Any?>
    assertEquals(settingsSig, identity["boundary"])
    assertEquals("tester-observed-failure", identity["invariant"])

    val contract =
      requireNotNull(engine.captureContractBug("state-preservation:rotation:draft", "state lost"))
    assertEquals("invariant", contract["oracle"])
    @Suppress("UNCHECKED_CAST")
    val contractIdentity = contract["findingIdentity"] as Map<String, Any?>
    assertEquals("state-preservation:rotation:draft", contractIdentity["invariant"])
    assertEquals(settingsSig, contractIdentity["boundary"])
  }
  ...
  fun jsonRoundTripsThroughDecoder() {
    ...
    @Suppress("UNCHECKED_CAST")
    val obj =
      Json.decode("{\"a\":1,\"b\":\"x\\\"y\",\"c\":[true,false,null],\"d\":{\"e\":\"f\"}}")
        as Map<String, Any?>
    assertEquals(1.0, obj["a"])
    assertEquals("x\"y", obj["b"])
    @Suppress("UNCHECKED_CAST") val c = obj["c"] as List<Any?>
    assertEquals(listOf(true, false, null), c)
    @Suppress("UNCHECKED_CAST") val d = obj["d"] as Map<String, Any?>
    assertEquals("f", d["e"])
  }
  ...
  fun configuredBuildIdentityIsAddedToContext() {
    val engine =
      Engine(ReproItConfig(appId = "example", buildVersion = "1.4.2", buildCommit = "abc123"))
    @Suppress("UNCHECKED_CAST") val build = engine.context()["build"] as Map<String, String>
  ...
  fun batchEnvelopeIncludesCtxWhenSetWithExactShape() {
    val engine = Engine(ReproItConfig(appId = "example"), now = { 1_717_939_200_123L })
    engine.setContexts(
      linkedMapOf("platform" to "android", "locale" to "en-US", "tz" to "America/New_York")
    )
    val ev =
      linkedMapOf<String, Any?>(
        "kind" to "error",
  ...
  fun fingerprintFieldsKeepsLabelDropsValue() {
    val out =
      Fingerprint.fingerprintFields(listOf("email" to "a@b.co", "#1" to "12345", "note" to ""))
  ...
  fun errorEventCarriesContextFingerprintAndOmitsWhenNull() {
    ...
    @Suppress("UNCHECKED_CAST") val context = ev["context"] as Map<String, Any?>
