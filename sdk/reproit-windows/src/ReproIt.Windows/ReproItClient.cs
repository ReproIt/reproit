...
namespace ReproIt.Windows
{
    public static class ReproItClient
    {
        ...
        private static void OnPointer(object sender, object args)
        {
            try
            {
                // Best-effort label: the most-recently focused / the source element's
                // accessible name. Full hit-testing differs per stack; we record the
                // event source's label when available, else tap:?. The engine turns
                // this into a tap:<label> edge consumed by the next state change.
                ...
                _engine?.NoteTap(clean);
            }
            catch
            {
                _engine?.NoteTap(null);
            }
        }
