...
namespace ReproIt.Core
{
    public static class Fingerprint
    {
        ...
        public static Dictionary<string, object> FingerprintValue(string value)
        {
            ...
            outMap["graphemes"] = GraphemeCount(codePoints);
        ...
        private static bool IsCombiningCp(int c)
        {
        ...
        private static int GraphemeCount(int[] codePoints)
        {
            int n = 0;
            bool joined = false;
            foreach (int c in codePoints)
            {
                ...
                if (IsCombiningCp(c) || (c >= 0xfe00 && c <= 0xfe0f))
                {
                    continue;
                }
                if (joined)
