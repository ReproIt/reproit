...
- **Taps.** A `UITapGestureRecognizer` is added to the key window with
  ...
  is hit-tested for the deepest tappable view under the touch point. `action` is
  a structural replay selector (`tap:key:<accessibilityIdentifier>` or
  `tap:role:<role>#<idx>`); `label` is optional display text.
- **Tappable** = a `UIControl`, a view carrying the `.button`/`.link`
  accessibility trait, or a view with a tap gesture recognizer.
...
it violated (a thrown error's `localizedDescription` becomes the finding
message). Registration is
idempotent by id and INERT in production: the predicate is stored but only
evaluated when the SDK detects it is running under the reproit fuzzer, which the
runner signals by launching the app with `REPROIT_FUZZ=1` in its process
environment (Appium XCUITest `processArguments.env`). Under the fuzzer, a
violated invariant is logged as a `REPROIT_INVARIANT` marker via `NSLog` (which
lands in the syslog the mobile runner scrapes). Call `invariant` after `start`.
...
The package ships a privacy manifest (`Sources/ReproIt/PrivacyInfo.xcprivacy`,
required by Apple for third-party SDKs since May 2024). SwiftPM copies it into the
target's generated resource bundle (`ReproIt_ReproIt.bundle`), which Xcode embeds
in your app at archive time; you do not add it to your target by hand. Xcode then
aggregates its declarations into your app's privacy report automatically.

The manifest declares:

- `NSPrivacyTracking = false` with an empty `NSPrivacyTrackingDomains`: the SDK
  does no cross-app or cross-site tracking and contacts only the `endpoint` you
  configure.
- `NSPrivacyAccessedAPITypes` is **empty**: the SDK calls no Apple required-reason
  APIs. It reads no file timestamps, no system boot time, no disk space, no active
  keyboard, and no `UserDefaults`. The crash spool only writes and reads its own
  files under Caches (`write` / `Data(contentsOf:)` / `removeItem`), none of which
  touch file-timestamp attributes, and event timestamps use wall-clock
  `Date().timeIntervalSince1970`, which is not a required-reason clock.
- `NSPrivacyCollectedDataTypes`: crash data, product interaction, and other
  diagnostic data, each marked unlinked (`Linked = false`) and non-tracking, for
  app-functionality purposes. This is a production telemetry SDK: when you set an
  `endpoint` it transmits state signatures, the interaction path, and crash/error
  events off device, so those categories are declared even though the values are
  structural and PII-safe (see the fingerprinting notes below). With no `endpoint`
  set, events stay on device via `onEvent`/debug and nothing is transmitted.

**Code signing for TestFlight and the App Store.** A SwiftPM source dependency
needs no separate provisioning profile: its code compiles into your app binary and
its resource bundle is embedded in, and signed with, the app under your team
identity during the normal Xcode archive and export. If you sign artifacts
yourself in CI, sign the embedded bundle with the same distribution identity and
the hardened runtime, for example:
...
Do not use `--deep`; sign nested artifacts individually. If instead you vend a
prebuilt `ReproIt.xcframework`, sign each slice the same way (`--options runtime`,
your distribution identity). Verify with `codesign --verify --strict` and confirm
the aggregated declarations in Xcode Organizer's privacy report before upload.
