...
final class ReproItTests: XCTestCase {
    ...
    func testErrorEventShape() {
        let ev = ReproItEvent.error(
            sig: "cae5a9d5",
            path: [ReproItStep(sig: "s1", action: "tap:key:submit", label: "Submit"),
    ...
    func testInvariantMarkerListsOnlyViolationsUnderFuzzer() {
        ...
        engine.invariant("cart") { throw NSError(domain: "app", code: 1, userInfo: [NSLocalizedDescriptionKey: "cart went negative"]) } // violated via throw
    ...
    func testCausalURLProtocolReplaysCanonicalRequestAndFailsClosedOnMiss() throws {
        let capsule: [String: Any] = ["exchanges": [[
        ...
        session.dataTask(with: URL(string: "https://api.test/config?b=2&a=1")!) { data, response, error in
            XCTAssertNil(error)
