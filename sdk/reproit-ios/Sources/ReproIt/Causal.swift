...
final class ReproItCausalURLProtocol: URLProtocol {
  ...
  override class func canInit(with request: URLRequest) -> Bool {
    ...
    if let prefix = excludePrefix, request.url?.absoluteString.hasPrefix(prefix) == true {
      return false
    }
    return true
  }
  ...
  override func startLoading() {
    ...
    if replaying {
      ...
      client?.urlProtocol(
        ...
        didFailWithError: NSError(
          domain: "ReproItCapsule", code: 1, userInfo: [NSLocalizedDescriptionKey: message]))
      return
    }
    ...
    loadingTask = URLSession(configuration: config).dataTask(with: mutable as URLRequest) {
      ...
      let responseHeaders = Self.redactHeaders(
        response.allHeaderFields.reduce(into: [String: String]()) {
  ...
  private static func mergeCapabilities(_ raw: String) {
    guard let path = ProcessInfo.processInfo.environment["REPROIT_CAPABILITIES_FILE"],
      ...
      let incoming = try? JSONSerialization.jsonObject(with: update) as? [String: Any]
    else { return }
  ...
  private static func redactHeaders(_ headers: [String: String]) -> [String: String] {
    Dictionary(
      uniqueKeysWithValues: headers.map { ($0.key, secret($0.key) ? "<reproit:secret>" : $0.value) }
    )
  }
  private static func bodyValue(_ data: Data?, headers: [String: String]) -> Any {
    guard let data, !data.isEmpty else { return NSNull() }
    if headers.first(where: { $0.key.lowercased() == "content-type" })?.value.contains("json")
      == true,
