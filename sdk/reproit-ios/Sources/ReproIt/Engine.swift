...
public final class ReproItEngine {
  ...
  public func recordError(
    message: String, stack: [String], source: String?, line: Int?,
    ...
    if let pending = pendingAction {
      pathCopy.append(
        ReproItStep(sig: sig, action: pending, label: cfg.redactLabels ? nil : pendingLabel))
    }
  ...
  @discardableResult
  public func captureBug(context: [String: Any]? = nil) -> Bool {
    lock.lock()
    if stopped {
      lock.unlock()
      return false
    ...
    if let pending = pendingAction {
      pathCopy.append(
        ReproItStep(
          sig: sig, action: pending,
          label: cfg.redactLabels ? nil : pendingLabel))
    }
    let trigger = pathCopy.last?.action ?? "load"
    let ev = ReproItEvent.testerCapture(
  ...
  @discardableResult
  public func captureContractBug(identity: String, message: String) -> Bool {
    lock.lock()
    if stopped {
      lock.unlock()
      return false
    ...
    let trigger = pathCopy.last?.action ?? "load"
    let ev = ReproItEvent.contractCapture(
  ...
  private struct FlushRequest {
    let req: URLRequest
    let batch: [ReproItEvent]
  }

  private func makeFlushRequest() -> FlushRequest? {
    lock.lock()
    guard let endpoint = cfg.endpoint, !buffer.isEmpty else {
      lock.unlock()
      return nil
    }
    let batch = buffer
