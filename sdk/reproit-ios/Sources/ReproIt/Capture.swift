...
// debounces snapshots, hit-tests taps for `tap:<label>` edges, and installs an
...
enum ReproItCapture {
    ...
    final class Target: NSObject {
        ...
        @objc func onTap(_ gr: UITapGestureRecognizer) {
            ...
            let label = ReproItCapture.accessibleLabelAt(point: point, in: window)
            ReproItCapture.engine?.setPendingAction(label.map { "tap:\($0)" } ?? "tap:?")
    ...
    /// `point` (in `window` coords). Hit-tests by walking the tree and keeping
    /// the deepest match, mirroring the Flutter SDK's `_labelAt`.
    static func accessibleLabelAt(point: CGPoint, in window: UIWindow) -> String? {
        var best: String?
        func visit(_ v: UIView) {
            ...
            if isTappable(v), let name = rawName(of: v),
               let n = ReproItName.normalize(name, maxLabelLen: 40) {
                best = n // children visited after parents => deepest wins
            }
