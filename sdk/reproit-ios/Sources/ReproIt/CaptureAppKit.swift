  ...
  enum ReproItAppKitCapture {
    ...
    private static func installClickRecognizer() {
      ...
      let recognizer = NSClickGestureRecognizer(
        target: Target.shared, action: #selector(Target.onClick(_:)))
    ...
    final class Target: NSObject {
      ...
      @objc func onClick(_ gr: NSClickGestureRecognizer) {
        guard let view = ReproItAppKitCapture.observedView ?? ReproItAppKitCapture.rootView() else {
          return
        }
        let point = gr.location(in: view)
        let target = ReproItAppKitCapture.tapTargetAt(point: point, in: view)
        ReproItAppKitCapture.engine?.setPendingAction(
    ...
    static func isTappable(_ v: NSView) -> Bool {
      ...
      let role = v.accessibilityRole()
      if role == .button || role == .link || role == .checkBox || role == .radioButton {
        return true
      }
    ...
    /// containing `point` (in `root` coords). Walks the tree keeping the deepest
    /// match, mirroring the UIKit / Flutter `tapTargetAt`.
    static func tapTargetAt(point: CGPoint, in root: NSView) -> (selector: String, label: String?)?
    {
      ...
      func visit(_ v: NSView) {
        ...
        if tappable {
          ...
          best = (selector, label)  // children after parents => deepest wins
        }
