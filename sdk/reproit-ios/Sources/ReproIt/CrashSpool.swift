...
public final class ReproItCrashSpool {
    ...
    public func stage(_ record: ReproItCrashRecord, appId: String) -> Bool {
        let obj: [String: Any] = [
            "appId": appId,
            "sig": record.sig,
            "path": record.path.map { ["sig": $0.sig, "action": $0.action] },
        ]
        ...
        if markerFd >= 0 { close(markerFd); markerFd = -1 }
    ...
    public func drainPending() -> ReproItCrashRecord? {
        ...
        let steps = (obj["path"] as? [[String: Any]])?.compactMap { step -> ReproItStep? in
            guard let s = step["sig"] as? String, let a = step["action"] as? String else { return nil }
            return ReproItStep(sig: s, action: a)
        } ?? []
