...
(function (global) {
  ...
  var ReproIt = {
    ...
    _path: [], // [{sig, action}] graph trail for repros
    ...
    init: function (opts) {
      ...
      // 3. interactions -> remember the action label, then re-snapshot
      addEventListener(
        "click",
        function (e) {
          var t = e.target;
          while (t && t !== document && !interactive(t)) t = t.parentElement;
          self._pending = "tap:" + (t && t !== document ? nameOf(t) || "?" : "?");
          self._settle(function () { self._observe(self._pending); });
        },
        true
      );

      // 4. error oracle: every uncaught error carries the graph PATH to it,
      //    which is exactly the seed of a deterministic repro test.
    ...
    _observe: function (action) {
      if (!this._on) return;
      var snap = snapshot(this._cfg);
      if (snap.sig === this._cur) return; // no state change
      var from = this._cur;
      this._cur = snap.sig;
      this._path.push({ sig: snap.sig, action: action });
