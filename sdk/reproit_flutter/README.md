...
- Records each tap by hit-testing the tapped semantics node. `action` is a
  structural replay selector (`tap:key:<key>` or `tap:role:<role>#<idx>`);
...
During fuzzing the zone client emits redacted causal exchanges. During `run` it
serves the capsule response and fails closed on any unmatched request. Bootstrap
traffic is phase 0; UI actions are 1-based. Custom clients created outside this
zone must use `ReproItCausalClient` explicitly.
