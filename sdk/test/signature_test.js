...
var valueVectors = vectors.filter(function (v) {
  return /^VALUE:/.test(v.description);
});
...
function sigByNeedle(needle) {
  var v = vectors.find(function (x) {
    return x.description.indexOf(needle) >= 0;
  });
...
check('chrome label with a value is NOT value-bearing', function () {
  ...
  var noValue = signatureOf('/home', {
    role: 'screen',
    children: [{ role: 'header', id: 'title' }],
  });
