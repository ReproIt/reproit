...
describe('causal fetch transport', () => {
  ...
  test('redacts before emitting the universal exchange marker', async () => {
    ...
    globalThis.fetch = jest.fn(
      async () =>
        new Response(JSON.stringify({ profile: { email: 'a@example.com' }, author: null }), {
          status: 200,
  ...
  test('replay is exact and fail closed', async () => {
    const exchange: CausalExchange = {
      ...
      protocol: 'https',
      method: 'GET',
      url: 'https://app.test/config?a=1&b=2',
      requestHeaders: {},
      status: 200,
    ...
    expect(await (await globalThis.fetch!('https://app.test/config?b=2&a=1')).json()).toEqual({
      enabled: true,
    });
  ...
  test('direct XMLHttpRequest is routed through fail-closed replay', async () => {
    ...
    const done = new Promise<void>((resolve, reject) => {
      xhr.onload = () => resolve();
      xhr.onerror = () => reject(new Error('XHR replay failed'));
    });
    xhr.open('GET', 'https://app.test/x?b=2&a=1');
    xhr.send();
    await done;
