...
export function nativeCausalCapsule(): { exchanges?: CausalExchange[] } | undefined {
  const globalValue = (globalThis as { __reproit_capsule?: { exchanges?: CausalExchange[] } })
    .__reproit_capsule;
...
function canonicalUrl(raw: string): string {
  try {
    ...
    const pairs = Array.from(url.searchParams.entries()).sort(
  ...
  if (value && typeof value === 'object') {
    ...
    for (const key of Object.keys(src).sort()) {
      ...
      out[key] = SECRET.test(key)
        ? `<reproit:${
            typeof child === 'string' ? `string:length=${[...child].length}` : typeof child
          }>`
        : redactCausal(child);
    }
    return out;
...
function headersOf(value: unknown): Record<string, string> {
  ...
  if (typeof h.forEach === 'function')
    h.forEach((v, k) => {
      out[k] = SECRET.test(k) ? '<reproit:secret>' : String(v);
    });
  else
...
export function installCausalFetch(options: {
  ...
  g.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (options.excludePrefix && url.startsWith(options.excludePrefix))
      return original(input, init);
    const actionIndex = options.actionIndex();
    if (actionIndex !== priorAction) {
      ordinal = 0;
      priorAction = actionIndex;
    }
    const method = String(
    ...
    try {
      ...
      responseBody = /json/i.test(contentType)
        ? redactCausal(await response.clone().json())
        : `<reproit:body>`;
    } catch {
    ...
    if (typeof init?.body === 'string') {
      try {
        requestBody = redactCausal(JSON.parse(init.body));
      } catch {
        requestBody = `<reproit:body:length=${init.body.length}>`;
      }
    }
    const exchange: CausalExchange = {
      id: `${actor}-${actionIndex}-${thisOrdinal}`,
      actor,
      actionIndex,
      ordinal: thisOrdinal,
      ...
      method,
      url,
      requestHeaders: headersOf(init?.headers),
  ...
  if (originalXhr) {
    class ReproItXMLHttpRequest {
      ...
      removeEventListener(kind: string, fn: EventListener): void {
        this.listeners.set(
          kind,
          (this.listeners.get(kind) || []).filter((item) => item !== fn),
        );
      }
      ...
      getAllResponseHeaders(): string {
        let out = '';
        this.responseHeaders.forEach((v, k) => {
          out += `${k}: ${v}\r\n`;
        });
        return out;
      }
      ...
      private fire(kind: string): void {
        ...
        const handler = this[`on${kind}` as 'onload'] as ((event: Event) => void) | null;
        handler?.(event);
        for (const fn of this.listeners.get(kind) || [])
          fn.call(this as unknown as EventTarget, event);
      }
      async send(body?: Document | XMLHttpRequestBodyInit | null): Promise<void> {
        try {
          const response = await g.fetch!(this.url, {
            method: this.method,
