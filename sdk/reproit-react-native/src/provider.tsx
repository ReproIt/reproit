 ...
 *
 *  1. Tap capture. It puts a transparent responder at the root of your tree
 ...
 *     the current tappable selectors and (best-effort) records the next edge as
 *     `tap:<selector>`. RN gives a library no synchronous hit-test by screen
 *     point, so when several tappables are present the target is a best-effort
...
export function ReproItProvider(props: ReproItProviderProps): React.ReactElement {
  ...
  const onTouchCapture = React.useCallback((): boolean => {
    // Best-effort tap target: pick from current tappables. When there is exactly
    ...
    const targets = ReproIt.tappableTargets();
    ReproIt.noteTapTarget(targets.length ? targets[0] : null);
