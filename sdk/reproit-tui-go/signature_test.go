...
func TestNumericValueClassesBounded(t *testing.T) {
	...
	// many repeated numbers -> a deduplicated set, still below the cap
	many := ""
	for n := 0; n < 50; n++ {
		many += itoa(n) + " "
	}
	if g := numericValueClasses(many); len(g) != 3 {
		t.Errorf("numeric value-class set must deduplicate to ZERO/POS1/POS2, got %v", g)
	}
