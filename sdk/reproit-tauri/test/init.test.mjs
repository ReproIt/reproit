...
function runtime(capsule, liveFetch) {
  ...
  const window = {
    ...
    __TAURI_INTERNALS__: { invoke: async (command, args) => {
      ...
      if (command.endsWith('|record_exchange')) { recorded.push(JSON.parse(args.line)); return null; }
...
test('document-start transport captures redacted JSON', async () => {
  const { window, recorded } = runtime(null, async () => new Response(JSON.stringify({ email: 'a@b.c', ok: true }), {
...
test('document-start transport replays and rejects unmatched requests', async () => {
  const capsule = { exchanges: [{ id: 'a-1-0', actor: 'a', actionIndex: 1, ordinal: 0,
    protocol: 'https', method: 'GET', url: 'https://api.test/config?a=1&b=2', status: 200,
    responseHeaders: { 'content-type': 'application/json' }, responseBody: { enabled: true }, required: true }] };
  const { window } = runtime(capsule, async () => { throw new Error('live network used'); });
  assert.deepEqual(await (await window.fetch('https://api.test/config?b=2&a=1')).json(), { enabled: true });
  await assert.rejects(window.fetch('https://api.test/miss'), /CAPSULE:MISS/);
});

test('XMLHttpRequest is captured and replayed through the same fail-closed adapter', async () => {
  const capsule = { exchanges: [{ id: 'a-1-0', actor: 'a', actionIndex: 1, ordinal: 0,
    protocol: 'https', method: 'POST', url: 'https://api.test/x?a=1&b=2', status: 201,
    responseHeaders: { 'content-type': 'application/json' }, responseBody: { ok: true }, required: true }] };
  const { window } = runtime(capsule, async () => { throw new Error('live network used'); });
  const xhr = new window.XMLHttpRequest(); xhr.responseType = 'json';
  const done = new Promise((resolve, reject) => { xhr.onload = resolve; xhr.onerror = () => reject(xhr._error); });
  xhr.open('POST', 'https://api.test/x?b=2&a=1'); xhr.send(JSON.stringify({ value: 1 })); await done;
  assert.equal(xhr.status, 201); assert.equal(JSON.stringify(xhr.response), '{"ok":true}');
  const miss = new window.XMLHttpRequest();
  const failed = new Promise((resolve) => { miss.onerror = resolve; });
  miss.open('GET', 'https://api.test/miss'); miss.send(); await failed;
