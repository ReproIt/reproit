...
struct FixtureApp: App {
  init() {
    ...
    URLSession.shared.dataTask(with: URL(string: "http://127.0.0.1:18766/bootstrap")!) {
      data, _, error in
      guard let path = ProcessInfo.processInfo.environment["REPROIT_NATIVE_RESULT_FILE"] else {
        return
      }
