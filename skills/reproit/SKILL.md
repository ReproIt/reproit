...
1. **Scan** (the default "what's wrong here"): `reproit scan [target]`. One
   ...
   Reproit maintains and refreshes its internal app model automatically; never
   ask the user to build or refresh a graph.
2. **Fuzz** (the DEEP search): `reproit fuzz [target]`. Combinatorially
