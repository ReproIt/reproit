...
void emitJson(String marker, Map<String, dynamic> payload) {
  emit('$marker ${jsonEncode(payload)}');
}
...
// and false-positive-free. Headless, the inset is only non-zero when the
// harness simulates an IME, so this never fires where no keyboard exists.
...
// WORKS HEADLESS (pure semantics facts, no timing). Identical to the
// simulator explorer's oracle; see templates/explorer.dart for the full
// rationale.
//
// The state rendered NOTHING (zero visible text labels, zero tappables, zero
// text fields, zero images) while the window has a non-zero size: the
// white-screen-of-death shape. FP guards: the caller runs this only after
// its settle; a null semanticsOwner means we cannot SEE the tree, not that
// the screen is blank, so it never fires (skip, silent); an image-only
// screen is NOT blank (the web scan's media check). Returns one
...
// WORKS HEADLESS (pure inset-vs-rect geometry, no timing). Identical to the
// simulator explorer's oracle; see templates/explorer.dart for the full
// rationale. An interactive control whose hit rect intersects a device
// safe-area inset (status bar / notch / Dynamic Island top, home indicator
// bottom, landscape notch / rounded corner left or right) is obscured or hard
// to tap. viewPadding is the inset in PHYSICAL px; _globalRect is the control's
// hit rect in the same space. A device/test with ZERO insets on every edge
// never fires; only tap-action nodes count; an intrusion under 1 logical px is
// flush-adjacent rounding, not a collision. Each item is {key, edge, by}.
...
// WORKS HEADLESS (pure label scan, no timing). Identical to the simulator
// explorer's oracle; see templates/explorer.dart for the full rationale.
//
// A VISIBLE label containing U+FFFD, the replacement character an encoding
// failure renders as tofu. The img/font reasons stay web-only, so the native
// `reason` vocabulary is a strict subset of the web one. Same walk and
// stable key grammar as detectContentBugs. Silent when no tofu is rendered.
...
// features override, set BEFORE the app first pumps so MediaQuery
// .disableAnimations is true run-wide). This pins animation-dependent timing so
// snapshots are stable across runs, exactly like the web context's reducedMotion
// emulation.
...
// Identical to the simulator explorer's setup; see templates/explorer.dart for
// the full rationale. Under REPROIT_DENY_PERMISSION, mock the permission_handler
// platform channel so every runtime request answers permanentlyDenied; record
// that a denial happened and mark each POST-denial screen with the marker. The
// Rust invariant fires only for a marked screen that is also a graph dead end.
...
/// true iff the sweep is active (a non-empty REPROIT_DENY_PERMISSION).
...
void main() {
  ...
  testWidgets('explore (headless)', (tester) async {
    ...
    // channel to deny every request BEFORE the app first pumps. Scoped to the run
    // (cleared in teardown). observe()'s marker is gated on this flag AND on a
    // denial having actually fired.
    ...
    Future<void> runSeed(FuzzCfg fuzz) async {
      ...
      Future<Snapshot> observe() async {
        ...
        if (seenStates.add(sig)) {
          ...
          // Headless view insets are only non-zero when the harness simulates
          // an IME, so this is silent in plain widget-test runs. Silent when
          // the screen is clean.
          ...
          // inset-vs-rect geometry in logical px (no timing), valid headless and
          ...
          // ALSO a graph dead end. Silent outside a denial sweep.
      ...
      for (var actions = 0; actions < budget && stuck < 3; actions++) {
        ...
        // UNDO-INVERSE: whether an Undo affordance already exists BEFORE X. Path B
        // only fires on an Undo that appeared because of X (attributable to it), so
