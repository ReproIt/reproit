#!/usr/bin/env bash
# Hermetic production-to-local acceptance for the Go SDK at capsule parity:
# capture a planted 5xx (a Transport upstream call and a database/sql query
# recorded with their responses) on the loopback fixture, then re-execute it
# with `check --exec` under the PORTABILITY bar: the replay legs run a binary
# built from a COPY of the checkout at a different absolute path, with no
# database and no upstream running (the SDK opens no outbound sockets in
# replay, so the network is effectively denied). Asserts the four-way verdict
...
(cd "$ROOT/$FIXTURE_REL" && go build -o "$WORK/fixture-capture" .)
MODE=capture CAPTURE_OUT="$WORK/capture.json" "$WORK/fixture-capture" >/dev/null
...
# PORTABILITY: the replay legs run a binary built from a copy of the checkout
# at a different absolute path. Only the SDK and the fixture are needed;
...
run_case "$WORK/tampered.json" "$SERVE" 3 \
  "go missing exchange diverges naming the call" \
  "DIVERGED"
