#!/usr/bin/env bash
# Flaky-CI wedge acceptance: a planted order-dependent test failure fires
...
# `reproit check <capsule> --exec "<test command>"` re-executes the exact
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/reproit-flaky-ci.XXXXXX")"
cleanup() { rm -rf "$WORK"; }
trap cleanup EXIT
...
run_case "$CAPSULE" "FIXED=1 node $TEST" 0 \
...
EOF
run_case "$WORK/tampered.json" "node $TEST" 3 \
...
echo "flaky-ci-e2e: flaky-vs-fixed distinguished, all four verdicts hold" \
