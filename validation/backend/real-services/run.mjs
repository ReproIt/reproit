...
function available(command) {
  const probe =
    process.platform === "win32" ? ["where", [command]] : ["sh", ["-c", `command -v ${command}`]];
...
async function runOne(name, mode, port) {
  ...
  try {
    ...
    if (validation.status !== 0) {
      throw new Error(
        `validator failed (${validation.status}): ${validation.stdout}${validation.stderr}`,
      );
    }
    return {
      service: name,
    ...
    if (logs.length)
      await writeFile(join(artifacts, host, `${name}-${mode}.log`), Buffer.concat(logs));
  }
}
...
for (const name of requested) {
  ...
  if (!available(definition.command)) {
    results.push({
      service: name,
      status: "unavailable",
      reason: `${definition.command} not installed`,
    });
    continue;
  }
