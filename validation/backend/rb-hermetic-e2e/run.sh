#!/usr/bin/env bash
# Hermetic production-to-local acceptance for the Ruby SDK at capsule
# parity: capture a planted 5xx (a Net::HTTP upstream call and a pg query
# through wrap_pg recorded with their responses) on the loopback fixture,
...
# replay, so the network is effectively denied). Asserts the four-way
...
MODE=capture CAPTURE_OUT="$WORK/capture.json" ruby "$ROOT/$FIXTURE_REL" >/dev/null 2>&1
...
run_case "$WORK/tampered.json" "$SERVE" 3 \
  "ruby missing exchange diverges naming the call" \
  "DIVERGED"
...
echo "rb-hermetic-e2e: all four verdicts hold ($CASES_RUN/$EXPECTED_CASES cases)"
