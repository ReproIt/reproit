#!/usr/bin/env bash
# Ruby flaky-CI wedge acceptance, cloned leg for leg from
# validation/backend/flaky-ci-e2e (the Node reference): a planted
# order-dependent test failure fires only under CI-like conditions (the CI
# legacy matrix leaks state into a shared upstream), the simulated CI run
# spools a test-trigger capsule, and `reproit check <capsule> --exec "<test
# command>"` re-executes the exact failing run under the PORTABILITY bar:
# the checkout is a COPY at a different absolute path and no recorded
# dependency runs (the SDK's replay wrappers serve everything in process; a
# real socket attempt would be a divergence, not a connection). Asserts the
...
# fix certifies (0), the revert reproduces again (1), and a deleted exchange
# diverges (3).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/reproit-rb-flaky-ci.XXXXXX")"
cleanup() { rm -rf "$WORK"; }
trap cleanup EXIT
...
if ! ruby "$TEST" >/dev/null 2>&1; then
...
run_case "$CAPSULE" "FIXED=1 ruby $TEST" 0 \
...
EOF
run_case "$WORK/tampered.json" "ruby $TEST" 3 \
...
echo "rb-flaky-ci-e2e: flaky-vs-fixed distinguished, all four verdicts hold" \
