#!/usr/bin/env bash
# Flaky-CI wedge acceptance for the PHP SDK, cloned leg for leg from
...
# a shared upstream), the simulated CI run spools a test-trigger capsule,
# and `reproit check <capsule> --exec "php <test file>"` re-executes the
# exact failing run under the PORTABILITY bar: the checkout is a COPY at a
# different absolute path and no recorded dependency runs (the SDK's replay
# boundary serves everything in process; a real socket attempt would be a
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/reproit-php-flaky-ci.XXXXXX")"
cleanup() { rm -rf "$WORK"; }
trap cleanup EXIT
...
assert exchanges[0]['exchange']['response']['body'] == {'value': {'rate': 0.25}}
EOF
...
run_case "$CAPSULE" "FIXED=1 php $TEST" 0 \
...
EOF
run_case "$WORK/tampered.json" "php $TEST" 3 \
  "deleted exchange diverges naming the call" \
  "DIVERGED"
if ! grep -q '"url":"http://127.0.0.1:19991/tax-rate"' "$WORK/out.txt"; then
...
echo "php-flaky-ci-e2e: flaky-vs-fixed distinguished, all four verdicts hold" \
