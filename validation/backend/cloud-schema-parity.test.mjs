...
test('local Cloud dogfood schema stays tied to real routes and response keys', { skip: !existsSync(mainPath) }, () => {
  const schema = JSON.parse(readFileSync(resolve(cloud, 'contracts/backend-openapi.json'), 'utf8'));
  ...
  for (const [method, route, constant] of routes) {
    ...
    assert.ok(schema.paths[openapiRoute]?.[method], `${method} ${openapiRoute} missing from Cloud artifact`);
    const escaped = route.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    assert.match(registry, new RegExp(`router_path:\\s*"${escaped}"`));
    assert.match(main, new RegExp(`\\.route\\(\\s*backend_contract::${constant}\\s*,\\s*${method}\\(`, 's'));
  }
