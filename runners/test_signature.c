...
static ReproItSig_Node* with_id(ReproItSig_Node* n, const char* id) { n->id = id; return n; }
static ReproItSig_Node* with_type(ReproItSig_Node* n, const char* t) { n->type = t; return n; }
static ReproItSig_Node* with_icon(ReproItSig_Node* n, const char* ic) { n->icon = ic; return n; }
static ReproItSig_Node* with_transient(ReproItSig_Node* n) { n->transient = true; return n; }
// Layer 2 value-state: attach a displayed value, or opt a node in as value-bearing.
static ReproItSig_Node* with_value(ReproItSig_Node* n, const char* v) { n->value = v; return n; }
static ReproItSig_Node* with_value_node(ReproItSig_Node* n) { n->value_node = true; return n; }
...
static ReproItSig_Node* v_login(void) { reset_pool(); return login_base(); }
static ReproItSig_Node* v_locale(void) { reset_pool(); return login_base(); }
...
static ReproItSig_Node* v_feed3(void) {
    ...
    for (int i = 0; i < 3; i++) kid(l, listitem());
    return l;
}
static ReproItSig_Node* v_feed5(void) {
    reset_pool();
    ReproItSig_Node* l = N("list");
    for (int i = 0; i < 5; i++) kid(l, listitem());
    return l;
}
...
static ReproItSig_Node* v_settings(void) { reset_pool(); return settings_tree(); }
static ReproItSig_Node* v_profile(void) { reset_pool(); return settings_tree(); }
...
static ReproItSig_Node* v_item42(void) { reset_pool(); return item_tree(); }
static ReproItSig_Node* v_item99(void) { reset_pool(); return item_tree(); }
...
static ReproItSig_Node* v_val_empty(void) { reset_pool(); return form_amount(""); }
static ReproItSig_Node* v_val_zero(void)  { reset_pool(); return form_amount("0"); }
static ReproItSig_Node* v_val_pos1(void)  { reset_pool(); return form_amount("3"); }
static ReproItSig_Node* v_val_grouped(void) { reset_pool(); return form_amount("1,234"); }
...
static ReproItSig_Node* v_counter0(void)   { reset_pool(); return counter("0"); }
static ReproItSig_Node* v_counter5(void)   { reset_pool(); return counter("5"); }
static ReproItSig_Node* v_counter_p3(void) { reset_pool(); return counter("3"); }
static ReproItSig_Node* v_counter_p7(void) { reset_pool(); return counter("7"); }
...
static int extra_value_checks(void) {
    ...
    if (strcmp(d.buf, "A:\n0:text@display") != 0) {
        fails++; printf("FAIL  opt-in (unflagged) descriptor = %s\n", d.buf);
    }
    with_value_node(t);
    reproit_sig_descriptor(NULL, t, &d);
    if (strcmp(d.buf, "A:\n0:text@display\nV:key:display=POS2") != 0) {
        fails++; printf("FAIL  opt-in (flagged) descriptor = %s\n", d.buf);
    }
    ...
    if (strcmp(d.buf, "A:\n0:screen;1:textfield*\nV:role:textfield#0=POS1;role:textfield#1=POS2") != 0) {
        fails++; printf("FAIL  keyless structural-index descriptor = %s\n", d.buf);
    }
    if (!fails) printf("ok    extra value-state checks (opt-in flag, keyless index)\n");
    return fails;
}
