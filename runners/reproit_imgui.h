...
static bool reproit_scn_http(const char* base, const char* method,
    ...
    if (s == REPROIT_SCN_BAD_SOCK) return false;
        ...
        if (n > 0 && (size_t)n < sizeof req) {
            ...
            if (off == (size_t)n) {
                ...
                if (body) {
                    ...
                    while (len && (body[0] == ' ' || body[0] == '\r' ||
                                   body[0] == '\n' || body[0] == '\t')) {
                        body++;
                        len--;
                    }
                    while (len && (body[len - 1] == ' ' || body[len - 1] == '\r' ||
                                   body[len - 1] == '\n' || body[len - 1] == '\t')) {
                        len--;
                    }
                    if (out && cap) {
...
struct State {
    ...
    struct Content { std::string id; std::string text; };
...
std::string displayText(const char* label) {
    ...
    std::string s(label);
    auto triple = s.find("###");
    if (triple != std::string::npos) return s.substr(0, triple);
    auto dbl = s.find("##");
    if (dbl != std::string::npos) return s.substr(0, dbl);
    return s;
}
#endif
...
void emit(const char* role, const char* label, bool interactive,
    ...
    if (!id.empty()) {
        ...
        if (!disp.empty()) g.frameContent.push_back({id, disp});
    }
#endif
}
...
void emitContentBugs(const std::string& sig) {
    ...
    struct Item { std::string key, reason, text; };
    ...
    for (const auto& c : g.frameContent) {
        ...
        if (!dedup.insert(dk).second) continue;
        items.push_back({c.id, reason, c.text.substr(0, 80)});
    }
    if (items.empty()) return;  // clean frame: stay silent
...
void scenarioStep() {
    if (!g.scnJoined) {
        ...
        if (dev && *dev) {
            ...
            char role[32];
            if (reproit_scn_http(g.scnBase.c_str(), "GET", "/claim", role, sizeof role)
                && role[0] && std::strncmp(role, "ERR", 3) != 0) {
                g.scnRole = role;
            } else {
                g.scnRole = "a";
            }
    ...
    std::string act(body);
    if (act.rfind("ACT\t", 0) == 0) act = act.substr(4);
    ...
    if (act.rfind("assert:", 0) == 0) {
        ...
        if (a.rfind("text=", 0) == 0) {
            ...
            bool found = false;
            for (const auto& c : g.frameContent) {
                if (c.text.find(want) != std::string::npos) { found = true; break; }
            }
            std::string quoted = "\"";
            appendJsonEscaped(quoted, want);
            quoted += "\"";
            std::printf("FUZZ:ASSERT %s text=%s actor=%s\n",
                        found ? "pass" : "fail", quoted.c_str(), g.scnRole.c_str());
        } else if (a.rfind("count:", 0) == 0) {
            std::string rest = a.substr(6);
            auto eq = rest.rfind('=');
            std::string finder = eq == std::string::npos ? rest : rest.substr(0, eq);
            long want = eq == std::string::npos
                ? 0 : std::strtol(rest.c_str() + eq + 1, nullptr, 10);
            long got = 0;
            for (int i = 0; i < g.nNodes; i++) {
                if (g.nodes[i].id && finder == g.nodes[i].id) got++;
            }
            std::printf("FUZZ:ASSERT %s count %s want=%ld got=%ld actor=%s\n",
                        got == want ? "pass" : "fail", finder.c_str(), want, got,
                        g.scnRole.c_str());
        } else {
    ...
    if (act.rfind("type:", 0) == 0) {
        ...
        if (finder.rfind("key:", 0) == 0) finder = finder.substr(4);
        bool present = false;
        for (int i = 0; i < g.nNodes; i++) {
            if (g.nodes[i].id && finder == g.nodes[i].id &&
                g.nodes[i].role && std::strcmp(g.nodes[i].role, "textfield") == 0) {
                present = true;
                break;
            }
        }
        if (!present) { miss(); ackNow(); return; }
        g.scnTypeId = finder;
    ...
    if (act.rfind("tap:", 0) == 0) {
        ...
        if (id.rfind("key:", 0) == 0) id = id.substr(4);
        bool present = false;
        for (const auto& t : g.frameTappables) {
            if (t == id) { present = true; break; }
        }
        if (!present) { miss(); ackNow(); return; }
        g.fireId = id;
        g.actions++;
...
void FrameEnd() {
    ...
    if (g.seen.insert(sig).second) {
        ...
        for (int i = 0; i < g.nNodes; i++) {
            ...
            if (!n.role || std::strcmp(n.role, "textfield") != 0 || !n.id || !n.id[0]) continue;
            ...
            if (std::strncmp(n.id, prefix, std::strlen(prefix)) == 0) {
                ...
                if (end) purposeLen = (size_t)(end - purpose);
            } else if (n.type && std::strcmp(n.type, "password") == 0) {
                purpose = "password";
                purposeLen = 8;
            }
            if (!purpose || !purposeLen) continue;
            if (!firstAuthElement) std::printf(",");
            ...
            for (const char* p = n.id; *p; ++p)
                if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') ||
                    (*p >= '0' && *p <= '9') || *p == '-' || *p == '_' || *p == '.') safeId += *p;
            std::printf("{\"sel\":\"key:%s\",\"role\":\"textfield\",\"label\":\"\",\"inputPurpose\":\"%.*s\"}",
