...
// ReproIt_Causal_Json and call ReproIt_Causal_Enable once; the shared adapter is
// declared by reproit_causal.h below and remains inert outside a Reproit run.
//
...
//
// IDENTICAL to the content-bug classifier block in runners/reproit_clay.h
...
// parity-critical signature core, so the parity test is unaffected.
//
...
// runners SEE the actual display strings the app draws (ImGui::Text/Button
// labels, Clay text elements), so we can scan that VISIBLE text for the SAME
// literal artifacts a stringify/template bug leaks to the screen, with the SAME
// classifier semantics + precedence, byte-for-byte:
...
// silent (no marker, no finding). The finding is addressed by the widget's
...
// ===========================================================================
// MULTI-ACTOR SCENARIO CORE (shared, self-contained: reproit_clay.h carries an
// identical block under the same guard)
//
...
void A11y(const char *idLabel, const char *role = nullptr, const char *name = nullptr,
          bool exposed = false);
...
struct State {
...
#ifndef REPROIT_TELEMETRY
  // Content-bug scan input (EXPLORE:CONTENTBUG): for every text-bearing widget
  // drawn this frame, its stable id (the finding key) + its DISPLAY text (what
  // the user sees, scanned for broken-content artifacts). Captured in document
  // order; deduped + classified + sorted at FrameEnd. Fuzz-only: a telemetry
  // build reports, it does not run oracles.
  struct Content {
    std::string id;
    std::string text;
  };
  std::vector<Content> frameContent;
  ...
  bool scn = false;
  std::string scnBase;        // conductor base URL
  std::string scnRole;        // this actor's role letter
  bool scnJoined = false;     // role claimed/announced
  bool scnAckPending = false; // a dispatched ACT awaits its /done ack
  std::string scnTypeId;      // pending type: target (stable id)
  std::string scnTypeValue;   // pending type: value to inject
#endif
};
State g;
...
void loadConfig() {
  ...
  const char *scn = std::getenv("REPROIT_SCENARIO_BARRIER");
  if (scn && *scn) {
    g.scn = true;
    g.scnBase = scn;
  }
...
std::string stableId(const char *label) {
  ...
  if (triple != std::string::npos)
    return s.substr(triple + 3);
  auto dbl = s.find("##");
  if (dbl != std::string::npos)
    return s.substr(dbl + 2);
  return s; // no marker: the whole label is the developer-chosen id
}

#ifndef REPROIT_TELEMETRY
// The DISPLAY text of an ImGui label: the part BEFORE "##"/"###" (what the user
// actually sees on screen). Unlike the signature, the content-bug oracle scans
// the VISIBLE text, so this is the string fed to the classifier. A bare label
// with no "##" marker IS its own display text. "Play##play" -> "Play";
// "${x}###k" -> "${x}"; "[object Object]" -> "[object Object]".
std::string displayText(const char *label) {
  if (!label)
    return std::string();
  std::string s(label);
  auto triple = s.find("###");
  if (triple != std::string::npos)
    return s.substr(0, triple);
  auto dbl = s.find("##");
  if (dbl != std::string::npos)
...
int alloc(const char *role, const std::string &id, const char *type, const char *icon,
  ...
  if (!id.empty()) {
    g.store[idx * 5 + 1] = id;
    n->id = g.store[idx * 5 + 1].c_str();
  }
  if (type && *type) {
    g.store[idx * 5 + 2] = type;
    n->type = g.store[idx * 5 + 2].c_str();
  }
  if (icon && *icon) {
    g.store[idx * 5 + 3] = icon;
    n->icon = g.store[idx * 5 + 3].c_str();
  }
  // A value of nullptr means "absent"; an empty string is a real (EMPTY) value.
  if (value) {
    g.store[idx * 5 + 4] = value;
    n->value = g.store[idx * 5 + 4].c_str();
  }
...
const char *gestureFor(const char *role) {
  if (role && std::strcmp(role, "textfield") == 0)
    return "field";
  return "button";
}

void emit(const char *role, const char *label, bool interactive, const char *type = nullptr,
          const char *icon = nullptr, const char *value = nullptr, bool valueNode = false) {
...
#ifndef REPROIT_TELEMETRY
  // Capture the widget's DISPLAY text for the content-bug scan, keyed by its
  // stable id (only addressable, keyed widgets yield replayable findings). The
  // text scanned is what the user sees (before "##"); the id is locale-stable.
  if (!id.empty()) {
    std::string disp = displayText(label);
    if (!disp.empty())
      g.frameContent.push_back({id, disp});
  }
#endif
}

// True for exactly the stable id the fuzzer chose this step.
bool fires(const char *label) { return !g.fireId.empty() && g.fireId == stableId(label); }
...
std::string sigImpl(bool dropValues) {
  ...
  if (g.nNodes == 0) {
    ...
    empty.type = nullptr;
    empty.icon = nullptr;
    empty.transient = false;
    empty.value = nullptr;
    empty.value_node = false;
    empty.n_children = 0;
  ...
  for (int i = 0; i < g.nNodes; i++) {
    saved[i] = g.nodes[i].value;
    g.nodes[i].value = nullptr;
  }
  ReproIt_Signature(anchor, &g.nodes[0], out);
  for (int i = 0; i < g.nNodes; i++)
    g.nodes[i].value = saved[i];
  return out;
}
...
std::string structuralSig() {
  ...
  if (structural == full)
    return full; // no value nodes: nothing to cap
...
void emitGroundtruth(const std::string &sig) {
  ...
  for (const auto &w : g.frameWidgets) {
    ...
    std::printf("%s{\"id\":\"%s\",\"operable\":true,\"gestureKind\":\"%s\","
                "\"a11y\":{\"rolePresent\":%s,\"namePresent\":%s,"
                "\"inTabOrder\":false,\"keyboardActivatable\":false}}",
                firstEl ? "" : ",", w.id.c_str(), w.gesture.c_str(), rolePresent ? "true" : "false",
...
// JSON control set (" \\ and the C0 controls), so the emitted CONTENTBUG marker is
// valid JSON the Rust parser (map.rs) reads. Deterministic, no locale.
...
// keyed widget's DISPLAY text with the shared classifier (web-exact semantics +
// precedence), dedupes by (key,reason), sorts by key then reason, and prints the
// items[] array. Emitted ONLY when at least one artifact is found, so a clean app
// stays silent (no marker, no finding) and the finding id is byte-identical run
// to run / on replay. The `text` is clipped to 80 bytes like the web runner.
void emitContentBugs(const std::string &sig) {
  // Collect (key, reason, clippedText), deduped by key|reason.
  struct Item {
    std::string key, reason, text;
  };
  std::vector<Item> items;
  std::set<std::string> dedup;
  for (const auto &c : g.frameContent) {
    const char *reason = reproit_cbug_reason(c.text.c_str());
    if (!reason)
      continue;
    std::string dk = c.id + "|" + reason;
    if (!dedup.insert(dk).second)
      continue;
    items.push_back({c.id, reason, c.text.substr(0, 80)});
  }
  if (items.empty())
  ...
  std::string line = "EXPLORE:CONTENTBUG {\"sig\":\"";
  line += sig;
  line += "\",\"items\":[";
  bool first = true;
  for (const auto &it : items) {
    ...
    first = false;
    line += "{\"key\":\"";
    appendJsonEscaped(line, it.key);
    line += "\",\"reason\":\"";
    appendJsonEscaped(line, it.reason);
    line += "\",\"text\":\"";
    appendJsonEscaped(line, it.text);
    line += "\"}";
  }
  line += "]}\n";
...
void reproit_imgui_install_crash_hook() {
  if (reproit_imgui_crash_installed)
    return;
  ...
  const char *pre = "EXCEPTION CAUGHT BY IMGUI APP \xe2\x95\xa1 IMGUI APP \xe2\x95\x9e\n"
                    "The following crash was raised by the app:\n"
                    "raised by signal ";
  size_t n = 0;
  while (pre[n])
    n++;
  if (n >= sizeof reproit_imgui_crash_pre)
    n = sizeof reproit_imgui_crash_pre - 1;
  ...
  for (int s : {SIGSEGV, SIGABRT, SIGBUS, SIGFPE, SIGILL})
    signal(s, reproit_imgui_crash_handler);
}
...
void reproit_imgui_build_crash_tail() {
  ...
  int w = std::snprintf(reproit_imgui_crash_post, sizeof reproit_imgui_crash_post,
                        ...
                        sig, act);
  reproit_imgui_crash_post_len =
      (w > 0 && (size_t)w < sizeof reproit_imgui_crash_post) ? (size_t)w : 0;
}
...
//   tap:<id>              fire the tappable with that stable id (fires())
//   type:<finder>=<v>     inject <v> into the InputText with that stable id
//   shoot:<name>          screenshot point (capture callback / GL default)
//   assert:text=<t>       this frame's drawn display text contains <t>
//   assert:count:<f>=<n>  this frame holds exactly <n> nodes with stable id <f>
//   auth:<acct>           unsupported (no session store); loud no-op
//   back / key:<Name>     no back affordance / key channel in immediate
//                         mode: FUZZ:MISS, so a cross-surface journey fails
...
void scenarioStep() {
  if (!g.scnJoined) {
    ...
    const char *dev = std::getenv("REPROIT_DEVICE");
    if (dev && *dev) {
      g.scnRole = dev;
    } else {
      char role[32];
      if (reproit_scn_http(g.scnBase.c_str(), "GET", "/claim", role, sizeof role) && role[0] &&
          std::strncmp(role, "ERR", 3) != 0) {
        g.scnRole = role;
      } else {
        g.scnRole = "a";
      }
    }
    std::printf("JOURNEY claimed role=%s\n", g.scnRole.c_str());
    std::fflush(stdout);
    g.scnJoined = true;
  }
  const std::string donePath = "/done?device=" + g.scnRole;
  char resp[64];
  if (g.scnAckPending) {
    // The previously dispatched action has settled and its STATE/EDGE were
    // just emitted above; ack it so the conductor advances.
    reproit_scn_http(g.scnBase.c_str(), "POST", donePath.c_str(), resp, sizeof resp);
    g.scnAckPending = false;
  }
  char body[2048];
  const std::string nextPath = "/next?device=" + g.scnRole;
  if (!reproit_scn_http(g.scnBase.c_str(), "GET", nextPath.c_str(), body, sizeof body)) {
    reproit_scn_sleep_ms(100); // conductor unreachable: retry next frame
    return;
  }
  if (std::strcmp(body, "DONE") == 0) {
    std::printf("JOURNEY DONE\nAll tests passed\n");
    std::fflush(stdout);
    g.done = true;
    return;
  }
  if (std::strcmp(body, "WAIT") == 0) {
    reproit_scn_sleep_ms(40); // not our turn / join barrier: poll cadence
    return;
  }
  std::string act(body);
  if (act.rfind("ACT\t", 0) == 0)
    act = act.substr(4);
  std::printf("FUZZ:ACT %s %s\n", g.scnRole.c_str(), act.c_str());
  std::fflush(stdout);
  // Non-dispatching verbs complete within this FrameEnd and ack immediately.
  auto ackNow = [&]() {
    reproit_scn_http(g.scnBase.c_str(), "POST", donePath.c_str(), resp, sizeof resp);
  };
  auto miss = [&]() {
    std::printf("FUZZ:MISS %s %s\n", g.scnRole.c_str(), act.c_str());
    std::fflush(stdout);
  };

  if (act.rfind("shoot:", 0) == 0) {
    ReproIt_Shoot(act.c_str() + 6);
    ackNow();
    return;
  }
  if (act.rfind("assert:", 0) == 0) {
    std::string a = act.substr(7);
    if (a.rfind("text=", 0) == 0) {
      std::string want = a.substr(5);
      bool found = false;
      for (const auto &c : g.frameContent) {
      ...
      std::string quoted = "\"";
      appendJsonEscaped(quoted, want);
      quoted += "\"";
      std::printf("FUZZ:ASSERT %s text=%s actor=%s\n", found ? "pass" : "fail", quoted.c_str(),
                  g.scnRole.c_str());
    } else if (a.rfind("count:", 0) == 0) {
      std::string rest = a.substr(6);
      auto eq = rest.rfind('=');
      std::string finder = eq == std::string::npos ? rest : rest.substr(0, eq);
      long want = eq == std::string::npos ? 0 : std::strtol(rest.c_str() + eq + 1, nullptr, 10);
      long got = 0;
      for (int i = 0; i < g.nNodes; i++) {
        if (g.nodes[i].id && finder == g.nodes[i].id)
          got++;
      }
      std::printf("FUZZ:ASSERT %s count %s want=%ld got=%ld actor=%s\n",
                  got == want ? "pass" : "fail", finder.c_str(), want, got, g.scnRole.c_str());
    } else {
      std::printf("FUZZ:ASSERT fail unsupported %s actor=%s\n", a.c_str(), g.scnRole.c_str());
    }
    std::fflush(stdout);
    ackNow();
    return;
  }
  if (act.rfind("auth:", 0) == 0) {
    std::printf("JOURNEY[a] step: auth-restore unsupported on the in-app "
                "hook; drive the login UI explicitly for %s\n",
                act.c_str());
    std::fflush(stdout);
    ackNow();
    return;
  }
  if (act.rfind("type:", 0) == 0) {
    std::string b = act.substr(5);
    auto eq = b.rfind('=');
    std::string finder = eq == std::string::npos ? b : b.substr(0, eq);
    std::string value = eq == std::string::npos ? "" : b.substr(eq + 1);
    if (finder.rfind("key:", 0) == 0)
      finder = finder.substr(4);
    bool present = false;
    for (int i = 0; i < g.nNodes; i++) {
      if (g.nodes[i].id && finder == g.nodes[i].id && g.nodes[i].role &&
          std::strcmp(g.nodes[i].role, "textfield") == 0) {
        present = true;
        break;
      }
    }
    ...
    g.scnTypeId = finder;
    g.scnTypeValue = value;
    g.settleFrames = 2;
    g.scnAckPending = true; // ack once the injection settles
    return;
  }
  if (act.rfind("tap:", 0) == 0) {
    std::string id = act.substr(4);
    if (id.rfind("key:", 0) == 0)
      id = id.substr(4);
    bool present = false;
    for (const auto &t : g.frameTappables) {
      if (t == id) {
        present = true;
        break;
      }
    ...
    g.actions++;
    g.settleFrames = 2;
    g.scnAckPending = true; // ack once the fire settles
    // Arm the jank window + crash attribution exactly like the fuzz picker.
  ...
  // back / key:<Name> / other cross-surface actions: no affordance here.
  miss();
  ackNow();
}
#endif // !REPROIT_TELEMETRY
  ...
  if (!g.done) {
    ...
    if (g.haveLastFrame) {
      ...
      if (!g.jankAction.empty()) {
        if (dur > g.windowMaxFrameMs)
          g.windowMaxFrameMs = dur;
        if (dur >= REPROIT_JANK_FLOOR_MS)
          g.windowJankFrames++;
      }
    }
    g.lastFrameMs = now;
...
bool InputText(const char *label, char *buf, size_t bufSize) {
  ...
  bool real = ImGui::InputText(label, buf, bufSize);
#ifndef REPROIT_TELEMETRY
  // Scenario `type:` injection: the conductor chose this field; write the
  // value into the app's own buffer and report a change, exactly as the
  // fires() mechanism reports a chosen button pressed.
  if (!g.scnTypeId.empty() && g.scnTypeId == stableId(label) && buf && bufSize) {
    std::snprintf(buf, bufSize, "%s", g.scnTypeValue.c_str());
    g.scnTypeId.clear();
    return true;
  }
#endif
  return real || fires(label);
}
...
void FrameEnd() {
  ...
  if (!g.jankAction.empty()) {
    if (g.windowMaxFrameMs >= REPROIT_HANG_FLOOR_MS) {
      std::printf("EXPLORE:HANG {\"from\":\"%s\",\"action\":\"%s\",\"bucket\":%d,\"count\":%d}\n",
                  g.jankFrom.c_str(), g.jankAction.c_str(), REPROIT_HANG_FLOOR_MS,
                  g.windowJankFrames);
      std::fflush(stdout);
    } else if (g.windowMaxFrameMs >= REPROIT_JANK_FLOOR_MS) {
      std::printf("EXPLORE:JANK {\"from\":\"%s\",\"action\":\"%s\",\"bucket\":%d,\"count\":%d}\n",
                  g.jankFrom.c_str(), g.jankAction.c_str(), REPROIT_JANK_FLOOR_MS,
                  g.windowJankFrames);
      std::fflush(stdout);
    }
    g.jankAction.clear();
  ...
  if (g.seen.insert(sig).second) {
    ...
    std::printf("EXPLORE:STATE {\"sig\":\"%s\",\"labels\":[],\"elements\":[", sig.c_str());
    bool firstAuthElement = true;
    const char *prefix = "reproit-purpose-";
    for (int i = 0; i < g.nNodes; i++) {
      const ReproItSig_Node &n = g.nodes[i];
      if (!n.role || std::strcmp(n.role, "textfield") != 0 || !n.id || !n.id[0])
        continue;
      const char *purpose = nullptr;
      size_t purposeLen = 0;
      if (std::strncmp(n.id, prefix, std::strlen(prefix)) == 0) {
        purpose = n.id + std::strlen(prefix);
        const char *end = std::strstr(purpose, "--");
        if (end)
          purposeLen = (size_t)(end - purpose);
      } else if (n.type && std::strcmp(n.type, "password") == 0) {
        purpose = "password";
        purposeLen = 8;
      }
      if (!purpose || !purposeLen)
        continue;
      if (!firstAuthElement)
        std::printf(",");
      firstAuthElement = false;
      // IDs created by REPROIT_INPUT_PURPOSE are identifier tokens. Emit
      // only the portable token alphabet so malformed app IDs cannot break
      // the marker JSON consumed by the host.
      std::string safeId;
      for (const char *p = n.id; *p; ++p)
        if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') || (*p >= '0' && *p <= '9') ||
            *p == '-' || *p == '_' || *p == '.')
          safeId += *p;
      std::printf(
          "{\"sel\":\"key:%s\",\"role\":\"textfield\",\"label\":\"\",\"inputPurpose\":\"%.*s\"}",
          safeId.c_str(), (int)purposeLen, purpose);
    }
    std::printf("]}\n");
    std::fflush(stdout);
    ...
    // scan of the drawn display text (no pixels, no timing), so it reproduces
    // on replay. Only emitted when a broken-content artifact was actually drawn,
    // so a clean app stays silent (no marker, no finding).
    emitContentBugs(sig);
#endif
  }
  ...
  if (g.soak) {
    ...
    if (rss) {
      std::printf("MEMORY:SAMPLE {\"t_ms\":%llu,\"heap_used\":%llu}\n",
                  (unsigned long long)(now - g.soakStart), (unsigned long long)rss);
      std::fflush(stdout);
    }
  }
#endif
  ...
  if (g.actions == 0) {
    std::printf("JOURNEY claimed role=a\n");
    std::fflush(stdout);
  }
