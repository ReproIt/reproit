...
import { groundtruthFromFiber, groundtruthFromNative, reconcileComposeControls } from './runner.mjs';
...
test('Compose semantics: keyed generic wrapper and semantic child become one control', () => {
  const elements = [
    { sel: 'key:save', key: 'save', role: 'node', label: '', bounds: [10, 20, 120, 48], nokey: false },
    { sel: 'role:button#0', key: null, role: 'button', label: 'Save', bounds: [10, 20, 120, 48], nokey: true },
  ];
  ...
  assert.deepStrictEqual(got.elements, [
    { sel: 'key:save', key: 'save', role: 'button', label: 'Save', bounds: [10, 20, 120, 48], nokey: false },
  ]);
...
test('Compose semantics: distinct overlapping controls are not collapsed', () => {
  const elements = [
    ...
    { sel: 'role:button#0', key: null, role: 'button', label: 'Open', bounds: [4, 0, 96, 40], nokey: true },
  ];
  const got = reconcileComposeControls(elements, []);
  assert.strictEqual(got.elements.length, 2);
  assert.deepStrictEqual(got.elements.map((e) => e.sel), ['key:menu', 'role:button#0']);
});
...
test('Compose semantics: role selectors are renumbered after duplicate removal', () => {
  const elements = [
    ...
    { sel: 'role:button#0', key: null, role: 'button', label: 'First', bounds: [0, 0, 100, 40], nokey: true },
    { sel: 'role:button#1', key: null, role: 'button', label: 'Second', bounds: [0, 50, 100, 40], nokey: true },
  ];
  const got = reconcileComposeControls(elements, []);
  assert.deepStrictEqual(got.elements.map((e) => e.sel), ['key:first', 'role:button#0']);
});
