...
test(
  'policy requires both pinned known-issue reproductions from the same ' + 'commit',
  async () => {
    const { report } = await runPolicy({ knownIssues: required });
    assert.equal(report.knownIssueBackedFindings.length, 2);
    await assert.rejects(
      runPolicy({ knownIssues: required.slice(1) }),
      /required pinned A2UI #1298 reproduction is missing/,
    );
    await assert.rejects(
      runPolicy({ knownIssues: required, issueCommit: 'different' }),
