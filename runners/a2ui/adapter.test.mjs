...
const catalog = {id: CATALOG_ID, document: {catalogId: CATALOG_ID, components: {Text: {}, Column: {}, Card: {}, TextField: {}, Button: {}}}};
...
function failingCapture(overrides = {}) {
  const stream = overrides.stream || [
    ...
    message('updateComponents', {surfaceId: 'main', components: [
...
test('captures hashes, ordered stream, renderer metadata, snapshots, actions, and exact oracle identity', () => {
  ...
  assert.deepEqual(capsule.stream.map(item => item.sequence), [0, 1, 2, 3, 4]);
...
test('deterministically replays an official A2UI v0.9 contact-form stream', async () => {
  const jsonl = await readFile(new URL('./fixtures/official-contact-form.jsonl', import.meta.url), 'utf8');
  const capsule = capture({
    protocolVersion: 'v0.9', protocolDocument, catalog, stream: parseJsonl(jsonl), renderer,
...
test('rejects protocol-order, graph, and integrity failures separately', () => {
  const capsule = failingCapture();
  capsule.stream[0].message = message('updateDataModel', {surfaceId: 'missing', path: '/', value: {}});
...
test('renderer matrix separates same-stream renderer divergence', () => {
  const react = failingCapture({observation: {status: 'fail', structuralSha256: sha256('react tree')}});
  const lit = failingCapture({renderer: {name: '@a2ui/lit', version: '0.10.1', platform: 'web'}, observation: {status: 'pass', structuralSha256: sha256('lit tree')}});
...
test('renderer matrix attributes the same exact failure on every renderer to the app', () => {
  ...
  const lit = failingCapture({renderer: {name: '@a2ui/lit', version: '0.10.1', platform: 'web'}, observation: {status: 'fail', structuralSha256}});
  const matrix = rendererMatrix([react, lit]);
...
test('renderer matrix separates agent nondeterminism for the same input', () => {
  ...
  const second = failingCapture({stream: [
