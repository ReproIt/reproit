...
async function fixture() {
  return JSON.parse(await readFile(new URL('./fixtures/adk-a2a-events-v0.9.json', import.meta.url), 'utf8'));
}

test('official ADK fixture passes through unchanged and cumulative createSurface is captured once', async () => {
  ...
  assert.deepEqual(evidence.capsule.stream.map(item => item.message), [
    input[0].status.message.parts[1].data,
    input[1].status.message.parts[1].data,
  ]);
});
...
test('extracts message and status-update variants with both official MIME spellings', () => {
  ...
  assert.deepEqual(extractAdkA2uiMessages({
    kind: 'message',
  ...
  assert.deepEqual(extractAdkA2uiMessages({
    ...
    status: {message: {parts: [{
      kind: 'data', data: create, metadata: {mimeType: 'application/json+a2ui'},
    }] }},
  }), [create]);
});
...
test('ignores ordinary A2A events, text parts, and non-A2UI data', () => {
  ...
  assert.deepEqual(extractAdkA2uiMessages({kind: 'status-update', status: {state: 'working'}}), []);
  assert.deepEqual(extractAdkA2uiMessages({kind: 'message', parts: [
...
test('unknown or ambiguous A2A envelopes fail closed', () => {
  const create = {version: 'v0.9', createSurface: {surfaceId: 'main', catalogId}};
  assert.throws(() => extractAdkA2uiMessages({kind: 'custom', parts: []}), /unsupported A2A event/);
  assert.throws(() => extractAdkA2uiMessages({kind: 'message', parts: [
    {kind: 'data', data: create},
  ]}), /no supported A2UI MIME/);
  assert.throws(() => extractAdkA2uiMessages({kind: 'message', parts: [{
    kind: 'data', data: create, mimeType: 'application\/a2ui+json',
    metadata: {mimeType: 'application\/json+a2ui'},
  }]}), /conflicting mimeType/);
  assert.throws(() => extractAdkA2uiMessages({kind: 'message', parts: [{
    kind: 'text', text: 'bad', mimeType: 'application\/a2ui+json',
  }]}), /not a data part/);
  assert.throws(() => extractAdkA2uiMessages({kind: 'message', parts: [{
    kind: 'data', data: [], mimeType: 'application\/a2ui+json',
  }]}), /must contain one object/);
});

test('changed duplicate createSurface is not hidden by cumulative-event deduplication', async () => {
  const input = await fixture();
  input[1].status.message.parts[0].data.createSurface.sendDataModel = true;
  const observed = instrumentAdkA2ui(input, common);
  for await (const _event of observed.events) { /* consume */ }
  const evidence = await observed.result;
  assert.equal(evidence.replay.status, 'invalid');
  assert.match(evidence.replay.protocolErrors.join('\n'), /already exists/);
});

test('default validator is pinned to the official web SDK and fails closed outside its contract', async () => {
  ...
  assert.deepEqual(
    await validateOfficialAdkA2uiMessages([], {protocolVersion: 'v1.0'}),
    ['unsupported ADK A2UI protocol version: v1.0'],
  );
});

test('fixture messages pass the validator built from the pinned official checkout', async t => {
  ...
  const catalog = JSON.parse(await readFile(
    `${checkout}/specification/v0_9/catalogs/basic/catalog.json`,
    'utf8',
  ));
  const protocolDocument = JSON.parse(await readFile(
    `${checkout}/specification/v0_9/json/server_to_client.json`,
    'utf8',
  ));
  assert.deepEqual(validateWithOfficialWebSdk(sdk, messages, {
    protocolVersion: 'v0.9',
    protocolDocument,
  ...
  assert.match(
    JSON.stringify(validateWithOfficialWebSdk(sdk, unknown, {
      protocolVersion: 'v0.9',
      protocolDocument,
      catalog: {id: catalogId, document: catalog},
    })),
  ...
  assert.match(
    JSON.stringify(validateWithOfficialWebSdk(sdk, messages, {
  ...
  assert.match(
    JSON.stringify(validateWithOfficialWebSdk(sdk, messages, {
      protocolVersion: 'v0.9', protocolDocument: mutatedProtocol,
      catalog: {id: catalogId, document: catalog},
    })),
...
test('buffered ADK preflight never delivers an invalid or unverified stream', async () => {
  ...
  await assert.rejects(
    preflightAdkA2ui(input, {
      ...common,
      maxRepairs: 0,
      deliver(messages) { deliveries.push(messages); },
    }),
...
test('buffered ADK preflight delivers only the independently verified repaired messages', async () => {
  const input = await fixture();
  input[1].status.message.parts[1].data.updateDataModel.value = 99;
  const deliveries = [];
