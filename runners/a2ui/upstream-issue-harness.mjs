...
if (!checkout) throw new Error('usage: upstream-issue-harness.mjs <checkout> [react-url] [lit-url]');
...
const protocolDocument = JSON.parse(await readFile(join(checkout, 'specification/v0_9_1/json/server_to_client.json'), 'utf8'));
const catalogDocument = JSON.parse(await readFile(join(checkout, 'specification/v0_9_1/catalogs/basic/catalog.json'), 'utf8'));
...
async function loadFixture(name) {
  ...
  await Promise.all(fixture.messages.map((message, index) => writeFile(join(messagesDirectory, `${index}.json`), JSON.stringify(message))));
...
try {
  ...
  for (const renderer of ['react', 'lit']) {
    ...
    const snapshots = await surface.locator('button').all().then(buttons => Promise.all(buttons.map(button => button.ariaSnapshot())));
    await page.close();
  ...
  for (const renderer of ['react', 'lit']) {
    ...
    assert.equal(runs.some(run => run.accessibleDescriptionPresent), false);
  }
  report.knownIssues.push({issue: 1410, status: 'reproduced', schemaValid: true, fixture: 'upstream-1410-accessible-action-purpose.json', renderers: accessibilityRuns});
  ...
  for (const renderer of ['react', 'lit']) {
    ...
    imageRuns[renderer] = await img.evaluate(element => {
      ...
      return {fit: style.objectFit, width: rect.width, height: rect.height, cssWidth: style.width, cssHeight: style.height};
    });
    await page.close();
  }
  ...
  report.knownIssues.push({issue: 1298, status: 'reproduced', schemaValid: true, fixture: 'upstream-1298-image-default.json', renderers: imageRuns});
