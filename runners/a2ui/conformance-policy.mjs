...
if (issues.upstream?.commit !== renderer.upstream?.commit) {
  throw new Error('renderer and known-issue reports were not captured from the same upstream commit');
}
for (const issue of [1298, 1410]) {
  if (!issues.knownIssues.some(finding => finding.issue === issue && finding.status === 'reproduced')) {
    throw new Error(`required pinned A2UI #${issue} reproduction is missing`);
  }
}
const knownIssueBackedFindings = issues.knownIssues.filter(finding => finding.status === 'reproduced');
