...
bool ReproIt_Clay_Clicked(Clay_ElementId id) {
    ...
    snprintf(stableName, sizeof stableName, "%.*s",
    ...
    if (!present && stableName[0] && rc.nNodes < REPROIT_CLAY_MAX) {
        ...
        if (n) reproit_clay_add_child(&rc.nodes[0], n);
    }
...
static void reproit_clay_scenario_step(void) {
    ...
    if (!rc.scnJoined) {
        ...
        if (dev && dev[0]) {
            ...
            char role[32];
            if (reproit_scn_http(rc.scnBase, "GET", "/claim", role, sizeof role) &&
                role[0] && strncmp(role, "ERR", 3) != 0) {
    ...
    if (strncmp(act, "shoot:", 6) == 0) {
        ...
        if (strncmp(a, "text=", 5) == 0) {
            ...
            for (int i = 0; i < rc.nContent && !found; i++) {
                if (strstr(rc.contentText[i], want)) found = true;
            }
            ...
            if (eq) {
                ...
                if (fl >= sizeof finder) fl = sizeof finder - 1;
            ...
            long got = 0;
            for (int i = 0; i < rc.nTaps; i++) {
                if (strcmp(rc.tapNames[i], finder) == 0) got++;
            }
            if (got == 0) {
                for (int i = 0; i < rc.nNodes; i++) {
                    if (rc.nodes[i].id && strcmp(rc.nodes[i].id, finder) == 0) got++;
                }
            }
            printf("FUZZ:ASSERT %s count %s want=%ld got=%ld actor=%s\n",
                   got == want ? "pass" : "fail", finder, want, got, rc.scnRole);
        } else {
        ...
        if (strncmp(name, "key:", 4) == 0) name += 4;
        int hit = -1;
        for (int i = 0; i < rc.nTaps; i++) {
            if (strcmp(rc.tapNames[i], name) == 0) { hit = i; break; }
        }
        if (hit >= 0) {
...
void ReproIt_Clay_FrameEnd(void) {
    ...
    if (!reproit_seen(sig)) {
        ...
        for (int i = 0; i < rc.nNodes; i++) {
            ...
            if (!n->role || strcmp(n->role, "textfield") != 0 || !n->id || !n->id[0]) continue;
            ...
            if (strncmp(n->id, purpose_prefix, strlen(purpose_prefix)) == 0) {
                ...
                if (end) purpose_len = (size_t)(end - purpose);
            } else if (n->type && strcmp(n->type, "password") == 0) {
                purpose = "password";
                purpose_len = 8;
            }
            if (!purpose || !purpose_len) continue;
            char safe_id[REPROIT_CLAY_STR];
            size_t out = 0;
            for (const char* p = n->id; *p && out + 1 < sizeof safe_id; p++) {
                if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') ||
                    (*p >= '0' && *p <= '9') || *p == '-' || *p == '_' || *p == '.') safe_id[out++] = *p;
            }
            safe_id[out] = 0;
            if (!first_auth_element) printf(",");
