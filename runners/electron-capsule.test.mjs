...
test('Electron WebSocket replay preserves SEND/RECV order and stays offline', async () => {
  ...
  try {
    ...
    let route;
    await installElectronWebSockets({ routeWebSocket: async (_pattern, handler) => { route = handler; } }, path);
    const sent = []; let receive;
    route({ url: () => 'wss://app.test/ws', send: (value) => sent.push(value), onMessage: (fn) => { receive = fn; }, close: () => assert.fail('closed') });
