...
static Op graph1Operable(QObject *o) {
  ...
  if (hasConnectedSignal(o, "clicked()") || hasConnectedSignal(o, "clicked(bool)"))
    return {true, "button"};
  if (hasConnectedSignal(o, "pressed()"))
    return {true, "longPress"};
  if (hasConnectedSignal(o, "toggled(bool)"))
    return {true, "toggle"};
  ...
  if (auto *w = dynamic_cast<FakeButton *>(o)) {
    (void)w;
    return {true, "button"};
  }
  return {false, ""};
...
static A11y graph2A11y(QObject *o) {
  ...
  if (iface) {
    ...
    a.rolePresent = (role != QAccessible::NoRole && role != QAccessible::Grouping &&
...
static void walkObject(QObject *o, int depth, QVector<GTElement> &elements, QStringList &sigTokens,
  ...
  if (op.operable || a.rolePresent) {
    ...
    if (!name.isEmpty()) {
      ...
      int idx = 0;
      while (roleSeen.contains(r + "#" + QString::number(idx)))
        idx++;
...
static void emitGroundTruth() {
  ...
  for (const auto &e : elements) {
    ...
    arr.append(QJsonObject{
        {"id", e.id}, {"operable", e.operable}, {"gestureKind", e.gestureKind}, {"a11y", a11y}});
  }
  QJsonObject payload{{"sig", sig}, {"focusTrap", focusTrap}, {"elements", arr}};
  out << "EXPLORE:GROUNDTRUTH " << QJsonDocument(payload).toJson(QJsonDocument::Compact) << "\n";
  out.flush();

  for (const auto &e : elements) {
    QStringList g;
    if (e.operable && !e.a11y.rolePresent)
      g << "NO_ROLE";
    if (e.operable && !e.a11y.inTabOrder)
      g << "KEYBOARD_UNREACHABLE";
    if (e.operable && !e.a11y.keyboardActivatable)
      g << "POINTER_ONLY";
