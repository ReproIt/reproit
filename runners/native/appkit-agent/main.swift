...
func graph2A11y(_ v: NSView, inKeyViewLoop: Bool) -> A11y {
  ...
  let keyboardActivatable =
    (v is NSControl && focusable)
    || (rolePresent && focusable)
  return A11y(
    rolePresent: rolePresent, namePresent: namePresent,
...
let realButton = NSButton(
  title: "Real Button", target: nil, action: #selector(NSApplication.terminate(_:)))
