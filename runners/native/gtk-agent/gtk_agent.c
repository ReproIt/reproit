...
static gboolean has_click_gesture(GtkWidget *w, const char **gesture_kind) {
  ...
  for (guint i = 0; i < n; i++) {
    ...
    if (GTK_IS_GESTURE_CLICK(c)) {
      *gesture_kind = "button";
      found = TRUE;
    } else if (GTK_IS_GESTURE_LONG_PRESS(c)) {
      *gesture_kind = "longPress";
      found = TRUE;
...
static gboolean graph1_operable(GtkWidget *w, const char **gesture_kind) {
  *gesture_kind = "";
  ...
  if (has_handler(G_OBJECT(w), "clicked")) {
    *gesture_kind = "button";
    return TRUE;
  }
  if (has_handler(G_OBJECT(w), "activate")) {
    *gesture_kind = "activate";
    return TRUE;
  }
  if (has_click_gesture(w, gesture_kind))
    return TRUE;
  return FALSE;
}
...
typedef struct {
  gboolean role_present, name_present, focusable, in_tab_order, keyboard_activatable;
} A11y;
...
static A11y graph2_a11y(GtkWidget *w) {
  ...
  a.role_present = (role != GTK_ACCESSIBLE_ROLE_GENERIC && role != GTK_ACCESSIBLE_ROLE_GROUP &&
                    role != GTK_ACCESSIBLE_ROLE_PRESENTATION && role != GTK_ACCESSIBLE_ROLE_NONE);
  ...
  if (acc) {
    ...
    a.role_present =
        (role != ATK_ROLE_UNKNOWN && role != ATK_ROLE_FILLER && role != ATK_ROLE_PANEL);
...
static void walk_widget(GtkWidget *w, int depth, WalkCtx *ctx) {
  ...
  if (operable || a.role_present) {
    ...
    if (operable)
      ctx->any_operable = TRUE;
    if (a.in_tab_order)
      ctx->any_tab = TRUE;

    if (!ctx->first)
      g_string_append_c(ctx->json_elements, ',');
    ctx->first = FALSE;
    ...
    g_string_append_printf(ctx->sig_tokens, "%s%d:%s@%s", ctx->sig_tokens->len ? ";" : "", depth,
                           role, id);

    g_string_append_printf(ctx->verdicts, "  %s operable=%s -> ", id, operable ? "true" : "false");
    if (operable && (!a.role_present || !a.in_tab_order || !a.keyboard_activatable)) {
      g_string_append(ctx->verdicts, "GAP(");
      gboolean f = TRUE;
      if (!a.role_present) {
        g_string_append(ctx->verdicts, "NO_ROLE");
        f = FALSE;
      }
      if (!a.in_tab_order) {
        g_string_append(ctx->verdicts, f ? "KEYBOARD_UNREACHABLE" : ",KEYBOARD_UNREACHABLE");
        f = FALSE;
      }
      if (!a.keyboard_activatable) {
        g_string_append(ctx->verdicts, f ? "POINTER_ONLY" : ",POINTER_ONLY");
      }
  ...
  if (GTK_IS_CONTAINER(w)) {
    ...
    for (GList *l = kids; l; l = l->next)
      walk_widget(GTK_WIDGET(l->data), depth + 1, ctx);
    g_list_free(kids);
  }
#endif
}
...
static void emit_groundtruth(GtkWidget *root) {
  ...
  printf("EXPLORE:GROUNDTRUTH {\"sig\":\"%s\",\"focusTrap\":%s,\"elements\":[%s]}\n", sig,
         focus_trap ? "true" : "false", ctx.json_elements->str);
  fflush(stdout);
...
static void noop_clicked(GtkButton *b, gpointer u) {
  (void)b;
  (void)u;
}
