...
test('detectContentBugs FIRES on an unrendered template binding and [object Object]', async () => {
...
test('detectContentBugs is SILENT on bare undefined/null/NaN words in real copy', async () => {
  ...
  try {
    ...
    await page.setContent('<!doctype html><html><body>' +
      ...
      '</body></html>');
    const items = await page.evaluate(detectContentBugs);
    assert.deepStrictEqual(items, [], `bare undefined/null/NaN prose must not fire, got ${JSON.stringify(items)}`);
  } finally {
    await browser.close();
  }
});
...
test('detectContentBugs SKIPS template/markup syntax inside a CODE context', async () => {
  ...
  try {
    ...
    await page.setContent('<!doctype html><html><body>' +
      ...
      '</body></html>');
    const items = await page.evaluate(detectContentBugs);
    assert.deepStrictEqual(items, [], `code-context syntax must not fire, got ${JSON.stringify(items)}`);
  } finally {
    await browser.close();
  }
});
...
test('detectContentBugs is deterministic across repeated captures', async () => {
  ...
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent('<!doctype html><html><body><span id="row">[object Object]</span>' +
...
test('detectContentBugs is SILENT on "[object Object]" mentioned in explanatory PROSE', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    ...
    await page.setContent('<!doctype html><html><body>' +
      '<span id="doc">The rendered result will be [object Object] because objects have no default string form.</span>' +
      '<span id="leak">Price: [object Object]</span>' +
      '<span id="bare">[object Object]</span>' +
      '</body></html>');
    const items = await page.evaluate(detectContentBugs);
    const keys = items.map((i) => i.key).sort();
    assert.ok(!keys.includes('id:doc'), `prose mentioning the phrase must not fire, got ${JSON.stringify(items)}`);
    assert.ok(keys.includes('id:leak'), 'a short field-label leak ("Price: [object Object]") must still fire');
...
test('detectContentBugs is SILENT on "{{ }}" template syntax mentioned in PROSE', async () => {
  ...
  try {
    ...
    // binding still fires.
    await page.setContent('<!doctype html><html><body>' +
      '<span id="doc">As with actions and transitions, animations can have parameters. The double {{curly}} braces are Svelte syntax.</span>' +
      '<span id="leak">{{ user.name }}</span>' +
      '<span id="dollar">Total: ${price}</span>' +
      '</body></html>');
    const items = await page.evaluate(detectContentBugs, []);
    const keys = items.map((i) => i.key);
    assert.ok(!keys.includes('id:doc'), `prose mentioning {{ }} must not fire, got ${JSON.stringify(items)}`);
...
test('detectContentBugs is SILENT on a reflected fuzzer-injected probe', async () => {
  ...
  try {
    ...
    const keys = items.map((i) => i.key);
    assert.ok(!keys.includes('id:reflected'), `a reflected fuzzer probe must not fire, got ${JSON.stringify(items)}`);
