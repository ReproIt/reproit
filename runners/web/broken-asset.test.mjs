...
if (!browserType)
  throw new Error(`unknown REPROIT_TEST_BROWSER: ${process.env.REPROIT_TEST_BROWSER}`);
...
async function criticalFacts(page) {
  ...
  page.on('response', async (resp) => {
    ...
    facts.set(resp.url(), {
      ...
      contentType: '',
      resourceType: type,
      optional: /\/analytics\.js/.test(resp.url()),
      sequence: observed,
    });
...
async function waitForFacts(page, facts, count) {
  ...
  for (
    let i = 0;
    i < 50 && [...facts.values()].some((fact) => !fact.contentType && !fact.failure);
    i++
  )
  ...
  try {
    ...
    await page.route('**/*', (route) => {
      ...
      return route.fulfill({
        ...
        contentType: 'text/html',
        body:
          '<!doctype html><link id="theme" rel="stylesheet" href="/missing.' +
          'css"><script id="app" src="/wrong-mime.js" defer></script><h1>Styled ' +
          'app</h1>',
      });
    });
    ...
    assert.ok(
      out.some(
        (item) =>
          item.key === 'key:id:theme' &&
          item.reason === 'stylesheet-http' &&
          /status=404/.test(item.detail),
      ),
      JSON.stringify(out),
    );
    assert.ok(
      out.some(
        (item) =>
          ...
          ((item.reason === 'script-mime' && /content-type=text\/html/.test(item.detail)) ||
            (item.reason === 'script-request' && /NS_ERROR_CORRUPTED_CONTENT/.test(item.detail))),
      ),
      JSON.stringify(out),
    );
  } finally {
...
test(
  'nested CSS imports and JavaScript modules identify the exact failed ' + 'dependency',
  async () => {
    const browser = await browserType.launch();
    try {
      const page = await browser.newPage();
      ...
      await page.route('**/*', (route) => {
        ...
        if (path === '/app.css')
          return route.fulfill({
            ...
            body: '@import "/theme.css"; body{color:black}',
          });
        if (path === '/theme.css')
          return route.fulfill({ status: 404, contentType: 'text/css', body: '' });
        if (path === '/app.js')
          return route.fulfill({
            ...
            body: 'import "./checkout.js";',
          });
        if (path === '/checkout.js')
          return route.fulfill({ status: 404, contentType: 'application/javascript', body: '' });
        return route.fulfill({
          status: 200,
          contentType: 'text/html',
          body:
            '<!doctype html><link id="theme-root" rel="stylesheet" href="/app.' +
            'css"><script id="app-root" type="module" src="/app.js"></' +
            'script><h1>Dependencies</h1>',
        });
      });
      await page.goto('http://asset.test/');
      ...
      assert.ok(
        out.some(
          (item) =>
            item.reason === 'stylesheet-import-http' &&
            /\/theme\.css/.test(item.detail) &&
            /root=http:\/\/asset\.test\/app\.css/.test(item.detail),
        ),
        JSON.stringify(out),
      );
      assert.ok(
        out.some(
          (item) =>
            item.reason === 'module-dependency-http' &&
            /\/checkout\.js/.test(item.detail) &&
            /root=http:\/\/asset\.test\/app\.js/.test(item.detail) &&
            /parent=unavailable/.test(item.detail),
        ),
        JSON.stringify(out),
      );
      assert.ok(
        !out.some((item) => /app\.(css|js) status=200/.test(item.detail)),
        JSON.stringify(out),
      );
    } finally {
      await browser.close();
...
test('multiple failed module roots never receive a guessed dependency parent', async () => {
  ...
  try {
    ...
    await page.route('**/*', (route) => {
      ...
      if (path === '/one.js')
        return route.fulfill({
          status: 200,
          contentType: 'application/javascript',
          body: 'import "./one-child.js";',
        });
      if (path === '/two.js')
        return route.fulfill({
          status: 200,
          contentType: 'application/javascript',
          body: 'import "./two-child.js";',
        });
      if (path.endsWith('-child.js'))
        return route.fulfill({ status: 404, contentType: 'application/javascript', body: '' });
      return route.fulfill({
        status: 200,
        contentType: 'text/html',
        body:
          '<!doctype html><script id="one" type="module" src="/one.js"></' +
          'script><script id="two" type="module" src="/two.js"></script>',
      });
    });
    await page.goto('http://asset.test/');
    await waitForFacts(page, facts, 4);
    ...
    assert.ok(
      dependencies.every(
        (item) => /parent=unavailable/.test(item.detail) && !/root=/.test(item.detail),
      ),
      JSON.stringify(out),
    );
  } finally {
...
test('healthy, third-party, and inactive resources stay silent', async () => {
  ...
  try {
    ...
    await page.route('**/*', (route) => {
      ...
      if (path === '/app.css')
        return route.fulfill({ status: 200, contentType: 'text/css', body: 'h1{color:green}' });
      if (path === '/app.js')
        return route.fulfill({
          status: 200,
          contentType: 'application/javascript',
      ...
      if (path === '/print.css')
        return route.fulfill({ status: 404, contentType: 'text/css', body: '' });
      if (path === '/alternate.css')
        return route.fulfill({ status: 404, contentType: 'text/css', body: '' });
      if (path === '/analytics.js')
        return route.fulfill({ status: 404, contentType: 'application/javascript', body: '' });
      if (new URL(req.url()).hostname === 'analytics.test')
        return route.fulfill({ status: 404, body: '' });
      return route.fulfill({
        status: 200,
        contentType: 'text/html',
        body:
          '<!doctype html><link rel="stylesheet" href="/app.css"><link ' +
          'rel="stylesheet" media="print" href="/print.css"><link rel="alternate ' +
          'stylesheet" title="Other" href="/alternate.css"><script src="/app.' +
          'js"></script><script src="/analytics.js"></script><script src="https://' +
          'analytics.test/tracker.js"></script><h1>Healthy</h1>',
      });
    });
    await page.goto('http://asset.test/');
...
test(
  'an intentionally cancelled critical request never falls through to ' + 'load failure',
  async () => {
    const browser = await browserType.launch();
    try {
      const page = await browser.newPage();
      ...
      await page.route('**/*', (route) =>
        route.fulfill({
          status: 200,
          contentType: 'text/html',
          body: '<!doctype html><script id="cancelled" src="/cancelled.js"></script>',
        }),
      );
      await page.goto('http://asset.test/');
      await page.evaluate(() =>
        document.getElementById('cancelled').dispatchEvent(new Event('error')),
      );
      const out = await page.evaluate(criticalResourceScan, [
        {
          url: 'http://asset.test/cancelled.js',
...
test('favicon/chrome icons are excluded; a genuine dead <img> still fires', async () => {
  ...
  try {
    ...
    assert.ok(
      !keys.some((d) => /favicon|apple-touch-icon/i.test(d)),
      'favicon/touch-icon must NOT be flagged',
    );
    assert.ok(
      keys.some((d) => /hero\.png/.test(d)),
      'a genuine dead content <img> must still fire',
    );
  } finally {
    await browser.close();
  }
...
test('a fuzzer-injected <img> is excluded by provenance; a real one is not', async () => {
  ...
  try {
    ...
    assert.ok(
      gkeys.some((d) => /photo\.png/.test(d)),
      'a real dead <img> alongside it must still fire',
    );
    ...
    assert.ok(
      naive.some((o) => o.reason === 'img' && o.detail === 'x'),
      'src=x fires when NOT attributable to a fuzz value',
    );
  } finally {
    await browser.close();
  }
...
test('fuzzer-typed tofu is excluded; genuine encoding tofu still fires', async () => {
  ...
  try {
    ...
    assert.ok(
      tofu.some((d) => /caf/.test(d)),
      'genuine app tofu must still fire',
    );
    assert.ok(
      !tofu.some((d) => /probe/.test(d)),
      'fuzzer-typed tofu must be excluded by provenance',
    );
  } finally {
    await browser.close();
  }
...
test(
  'an off-screen / hidden / zero-size broken img is NOT flagged (Next.js /' +
    '_next/image residual)',
  async () => {
    const browser = await browserType.launch();
    try {
      const page = await browser.newPage();
      ...
      <img id="hidden" src="/_next/image?url=%2Fh.png"
      ...
      <div style="position:absolute;top:5000px">
        <img id="offscreen" src="/_next/image?url=%2Foff.png" width="200" height="120">
      </div>
    `);
      await page.waitForTimeout(400);
      const out = await page.evaluate(brokenAssetScan, []);
      ...
      assert.ok(
        imgs.some((d) => /visible\.png/.test(d)),
        'a visibly-rendered broken img still fires',
      );
      assert.ok(
        !imgs.some((d) => /(%2Fh|%2Fz|%2Foff)/.test(d)),
        'hidden / zero-size / off-screen broken imgs must NOT fire',
      );
    } finally {
      await browser.close();
    }
...
test(
  'a webfont whose FontFace errored but text renders fine is NOT flagged ' +
    '(no font-status path)',
  async () => {
    const browser = await browserType.launch();
    try {
    ...
    `);
      ...
      const out = await page.evaluate(brokenAssetScan, []);
      assert.ok(
        !out.some((o) => o.reason === 'font'),
        'a font-status error with clean rendered text must NOT fire',
      );
    } finally {
      await browser.close();
    }
