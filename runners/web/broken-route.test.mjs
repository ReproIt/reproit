...
test('isAssetPath excludes downloads/assets but keeps navigable pages', () => {
  ...
  for (const p of ['/files/app.zip', '/docs/manual.pdf', '/mac/app.dmg', '/win/setup.exe',
    '/release.tar.gz', '/bundle.js', '/styles/site.css', '/img/logo.svg', '/data/export.csv']) {
  ...
  for (const p of ['/', '/pages/examples/invoice.html', '/index2.html', '/jobs/role/123',
    '/settings', '/cart/checkout']) {
    assert.equal(isAssetPath(p), false, `${p} is a page -> must be probed`);
  }
});

test('a GET probe sees 200 where a HEAD probe would see 501 (the AdminLTE FP), and 404 stays dead', async () => {
  ...
  try {
    ...
    await page.route('**/*', (route) => {
      ...
      if (url.pathname === '/index2.html') {
        return route.fulfill(method === 'HEAD' ? { status: 501 } : { status: 200, contentType: 'text/html', body: 'ok' });
      }
      if (url.pathname === '/invoice.html') {
        return route.fulfill({ status: 404, contentType: 'text/html', body: 'gone' });
      }
      return route.fulfill({ status: 200, contentType: 'text/html', body: '<!doctype html><title>root</title>' });
    });
    ...
    assert.equal(isDeadRouteStatus(seen.index2Get), false, 'a live GET-200 page is NOT a broken route');
