...
// Reset prevents a state-persisting app from leaking state between seeds.
// This is browser-backed through Playwright.
...
test(
  'clearClientStorage wipes localStorage / sessionStorage / IndexedDB and ' +
    'calls the app reset hook',
  async () => {
    ...
    try {
      ...
      await page.route('**/*', (r) =>
        r.fulfill({ contentType: 'text/html', body: '<!doctype html><body>persist</body>' }),
      );
      ...
      await page.evaluate(async () => {
        ...
        window.__reproitReset = () => {
          window.__reproitResetCalled = true;
        };
        if (window.indexedDB) {
      ...
      const after = await page.evaluate(async () => {
        ...
        try {
          ...
          /* older engines: skip the IDB assertion */ dbNames = null;
        }
        return {
      ...
      assert.strictEqual(
        after.hookCalled,
        true,
        'an app-provided window.__reproitReset() hook must be called',
      );
      if (Array.isArray(after.dbNames)) {
        assert.ok(
          !after.dbNames.includes('appdb'),
          `IndexedDB database must be deleted, saw ${after.dbNames}`,
        );
      }
    } finally {
      await browser.close();
