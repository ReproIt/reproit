...
import { exceptionIsFirstParty, exceptionIsBenign, exceptionThrownInTracker, exceptionIsNonDeterministic } from './runner.mjs';
...
test('drops a throw inside a self-hosted Adobe analytics script (awshome_s_code.js)', () => {
  ...
  const stack =
    'Error: AWSMA: The pageview call cannot be made because the CDP request did not return\n' +
    '    at f (https://a0.awsstatic.com/languages/awshome_s_code.js:1:2)\n' +
    '    at g (https://docs.aws.amazon.com/assets/r/main.js:3:4)';
  assert.equal(exceptionIsFirstParty(stack, 'https://docs.aws.amazon.com'), true); // origin filter alone keeps it
...
test('drops a stackless JSON-parse-got-HTML rejection (third-party fetch)', () => {
  assert.equal(exceptionIsNonDeterministic(`Unexpected token '<', "<!DOCTYPE "... is not valid JSON`, ''), true);
...
test('KEEPS an app-code JSON.parse bug that carries an app stack frame', () => {
  const stack = 'SyntaxError: "x" is not valid JSON\n    at https://app.com/assets/parse.js:2:2';
  assert.equal(exceptionIsNonDeterministic('"x" is not valid JSON', stack), false); // has a frame -> not dropped here
});

test('does NOT drop an ordinary app crash by the non-deterministic filter', () => {
  assert.equal(exceptionIsNonDeterministic("Cannot read properties of null (reading 'x')", ''), false);
});
