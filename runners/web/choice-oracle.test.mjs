...
test(
  ...
  async () => {
    ...
    try {
      ...
      <div id="unrelated">
        <button aria-pressed="true">Small</button><button>Medium</button><button>Large</button>
      </div>
      <script>
        window.clicked = [];
...
test('layoutDelta: horizontal overflow + keyed anchor displacement', () => {
  ...
  // Anchors are matched by KEY, not index: a bar that un-pins between
  // measurements drops out and contributes nothing, and the remaining anchors
  // still pair with their own prior positions.
  const dropped = { hOverflow: 0, anchors: [['nav:1', 48, 0]] };
  assert.strictEqual(layoutDelta(base, dropped), 0);
  assert.strictEqual(layoutDelta(dropped, base), 0);
});
