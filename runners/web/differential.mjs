...
async function applyAction(page, act) {
  if (act.startsWith('key:')) {
    const map = { Down: 'ArrowDown', Up: 'ArrowUp', Right: 'ArrowRight', Left: 'ArrowLeft',
    ...
    await page.getByText(label, { exact: false }).first().click({ timeout: 2000 }).catch(() => {});
...
const origin = (() => { try { return new globalThis.URL(URL).origin; } catch { return ''; } })();
...
async function captureEngine(engine, replay) {
  ...
  page.on('pageerror', (e) => console_all.push({ type: 'pageerror', text: String(e).slice(0, 240), url: URL }));
  await page.goto(URL, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {});
  for (const a of replay) { await applyAction(page, a); await page.waitForTimeout(120); }
  ...
  for (const ms of FRAMES) {
    ...
    if (buf) { const p = `${OUT}/${engine}_${ms}.png`; writeFileSync(p, buf); frames[ms] = p; }
  }
  ...
  const boxes = await page.evaluate(() => {
    ...
    for (const el of document.querySelectorAll(sel)) {
      ...
      if (!out[key]) out[key] = { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) };
    }
    return out;
  }).catch(() => ({}));
  ...
  const firstParty = [...new Set(
    console_all.filter((c) => !c.url || c.url.startsWith(origin) || c.url.startsWith('http') === false || c.url === URL)
      .map((c) => `[${c.type}] ${c.text}`),
  )];
  await browser.close();
...
function visualDiff(aPath, bPath, outPath) {
  ...
  if (a.width !== b.width || a.height !== b.height) return { ratio: 1, ratioRaw: 1, diffPath: null, sizeMismatch: true };
  ...
  const mismatchRaw = pixelmatch(a.data, b.data, null, width, height, { threshold: 0.1, includeAA: true });
...
async function main() {
  ...
  for (const e of ENGINES) {
    if (!BY[e]) { emit(`  skip unknown engine ${e}`); continue; }
    results.push(await captureEngine(e, replay));
  }
  if (results.length < 2) { emit('Need >=2 engines to diff.'); emit('All tests passed'); return; }

  const ref = results[0];
  const findings = [];
  const report = { url: URL, headless: HEADLESS, engines: ENGINES, reference: ref.engine, frames: FRAMES, comparisons: [] };

  for (const r of results.slice(1)) {
    emit(`\n=== ${ref.engine} (reference) vs ${r.engine} ===`);
    const cmp = { engine: r.engine, visual: [], consoleOnlyHere: [], consoleOnlyRef: [], layout: [] };
    ...
    for (const ms of FRAMES) {
      ...
      emit(
        `  VISUAL ${ms}ms: ${pct}% sharp (non-AA) · meanΔ ${d.meanDelta.toFixed(2)}/255 (gradient)${flag ? '  ⚠ FLAG' : ''}`,
      );
      if (sharp) findings.push(`${r.engine} sharp-diverges ${pct}% from ${ref.engine} at ${ms}ms`);
      if (smooth)
        findings.push(
          `${r.engine} renders the animation/gradient differently from ${ref.engine} at ${ms}ms (meanΔ ${d.meanDelta.toFixed(2)})`,
        );
    }
    ...
    if (cmp.consoleOnlyHere.length) {
      ...
      findings.push(`${r.engine} has ${cmp.consoleOnlyHere.length} first-party console error(s) absent in ${ref.engine}`);
    }
    // LAYOUT
    for (const key of Object.keys(ref.boxes)) {
      const a = ref.boxes[key], b = r.boxes[key];
      if (!b) { cmp.layout.push({ key, missing: true }); continue; }
...
function writeReportHtml(path, report, results) {
  ...
  const rows = report.frames.map((ms) => {
    const cells = eng.map((e) => `<td><img src="${e}_${ms}.png"></td>`).join('');
    const diffs = report.comparisons.map((c) => {
      const v = c.visual.find((x) => x.ms === ms);
      const pct = v ? (v.ratio * 100).toFixed(2) : '-';
      return `<td><div class="lbl">${c.engine} Δ ${pct}%${v && v.flag ? ' ⚠' : ''}</div><img src="diff_${c.engine}_${ms}.png"></td>`;
    }).join('');
    return `<tr><th>${ms}ms</th>${cells}${diffs}</tr>`;
  }).join('');
  const head = `<tr><th></th>${eng.map((e) => `<th>${e}</th>`).join('')}${report.comparisons.map((c) => `<th>diff: ${report.reference}↔${c.engine}<br><small>non-AA</small></th>`).join('')}</tr>`;
