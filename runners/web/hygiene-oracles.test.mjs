...
import {
  ...
  focusLossArm, focusLossCheck, scrollRoundTripScan,
  zoomTappableKeys, zoomReflowScan,
} from './hygiene-oracles.mjs';

test('zoom reflow ignores offscreen controls and responsive breakpoint replacement', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent('<!doctype html><style>' +
      '#desktop{width:120px;height:32px}@media(max-width:500px){#desktop{position:absolute;left:-9999px;width:0}#mobile{display:block}}' +
      '#mobile{display:none}</style><body>' +
      '<a id="skip" href="#main" style="position:absolute;left:-9999px">Skip</a>' +
      '<button id="desktop">Search</button><button id="mobile">Menu</button><main id="main">Main</main>');
    const pre = await page.evaluate(zoomTappableKeys);
    assert.ok(!pre.some((x) => x.key === 'key:id:skip'), 'offscreen skip link must not enter baseline');
    await page.setViewportSize({ width: 400, height: 600 });
    assert.deepStrictEqual(await page.evaluate(zoomReflowScan, pre), [], 'responsive replacement is not collapse');
...
test('zoom reflow still reports a genuinely collapsed in-place control', async () => {
  ...
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent('<!doctype html><style>@media(max-width:500px){#buy{width:0!important;height:0!important;padding:0!important;border:0!important;overflow:hidden}}</style>' +
      '<button id="buy" style="position:absolute;left:20px;top:20px;width:100px;height:32px">Buy</button>');
    const pre = await page.evaluate(zoomTappableKeys);
    await page.setViewportSize({ width: 400, height: 600 });
    const out = await page.evaluate(zoomReflowScan, pre);
    assert.ok(out.some((x) => x.key === 'key:id:buy' && x.kind === 'collapsed'), JSON.stringify(out));
  } finally { await browser.close(); }
});

test('scroll round-trip fires only for stable same-shape rows with changed content', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent('<!doctype html><body style="margin:0"><div id="list" style="width:400px;height:180px;overflow-y:auto"></div>');
    await page.locator('#list').evaluate((list) => {
      for (let i = 0; i < 20; i++) {
        const row = document.createElement('div'); row.style.height = '60px'; row.textContent = 'Row ' + i; list.appendChild(row);
      }
      let bottom = false;
...
test('scroll round-trip ignores a leaf-to-container sample while virtualization settles', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent('<!doctype html><body style="margin:0"><div id="list" style="width:400px;height:180px;overflow-y:auto"></div>');
    await page.locator('#list').evaluate((list) => {
      for (let i = 0; i < 20; i++) {
        const row = document.createElement('span'); row.style.cssText = 'display:block;height:60px'; row.textContent = 'Row ' + i; list.appendChild(row);
      }
      let bottom = false;
      list.addEventListener('scroll', () => {
        if (list.scrollTop > 500) bottom = true;
        if (bottom && list.scrollTop === 0) list.replaceChildren(Object.assign(document.createElement('div'), { textContent: 'Loading all virtual rows together' }));
      });
    });
    const result = await page.evaluate(scrollRoundTripScan);
...
test('occlusion FIRES on a real button covered by a foreign opaque overlay', async () => {
  ...
  try {
    ...
    await page.setContent('<!doctype html><body style="margin:0">' +
      '<button id="real-btn" style="position:absolute;left:60px;top:60px;width:120px;height:40px">Buy now</button>' +
      '<div id="stray" style="position:absolute;left:50px;top:50px;width:150px;height:70px;background:#c00;z-index:99"></div>' +
      '</body>');
...
test('occlusion is SILENT on a control inside a CLOSED flyout (opacity:0 ancestor)', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    ...
    await page.setContent('<!doctype html><body style="margin:0">' +
      ...
      '<button style="position:absolute;inset:0;width:120px;height:32px">Ecosystem</button>' +
      '<div style="opacity:0;position:absolute;top:0;left:0">' +
      '<a id="flyout-link" href="/x" style="display:block;width:120px;height:32px">Awesome Vue</a>' +
      '</div></div></body>');
    const out = await page.evaluate(occlusionScan);
    assert.ok(
...
test('occlusion is SILENT on an off-screen sr-only skip-link', async () => {
  ...
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent('<!doctype html><body style="margin:0">' +
      '<a id="skip" href="#main" style="position:absolute;left:-9999px;top:0;width:120px;height:30px">Skip to content</a>' +
...
test('occlusion is SILENT on a link scrolled OUT of an overflow:auto container (clipped away)', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    ...
    await page.setContent('<!doctype html><body style="margin:0">' +
      '<div style="position:absolute;left:0;top:0;width:400px;height:400px;background:#0a2;z-index:0"></div>' +
      '<div style="position:absolute;left:0;top:0;overflow:auto;width:200px;height:80px;z-index:1">' +
    ...
    assert.ok(
      ...
      `a link scrolled out of an overflow:auto viewport must not fire, got ${JSON.stringify(out)}`,
    );
  } finally {
    await browser.close();
  }
...
test('occlusion is SILENT on a control inside a CLOSED <details> (collapsed disclosure)', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    ...
    await page.setContent('<!doctype html><body style="margin:0">' +
      '<div style="position:absolute;left:0;top:0;width:400px;height:200px;background:#c00;z-index:0"></div>' +
      '<details style="position:absolute;left:0;top:0;z-index:1"><summary>Sections</summary>' +
      '<a id="menu-link" href="/styling" style="display:block;width:150px;height:30px">Styling</a>' +
      '</details></body>');
    const out = await page.evaluate(occlusionScan);
    assert.ok(
...
test('occlusion STILL FIRES on a buried link inside the SUMMARY of a closed <details>', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    ...
    // over-suppress it.
    await page.setContent('<!doctype html><body style="margin:0">' +
      '<div style="position:absolute;left:0;top:0;width:400px;height:60px;background:#06c;z-index:5"></div>' +
      '<details style="position:absolute;left:0;top:0;z-index:0"><summary>' +
      '<a id="sum-link" href="/x" style="display:inline-block;width:120px;height:30px">Open</a>' +
...
test('occlusion STILL FIRES on an in-view control opaquely covered inside a scroll container', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    ...
    await page.setContent('<!doctype html><body style="margin:0">' +
      ...
      '<button id="inview" style="position:absolute;left:20px;top:20px;width:100px;height:40px">Save</button>' +
      '<div style="position:absolute;left:10px;top:10px;width:150px;height:80px;background:#06c"></div>' +
      '</div></body>');
    const out = await page.evaluate(occlusionScan);
    assert.ok(
      out.some((o) => o.target.includes('inview')),
      `an in-view control opaquely covered inside a scroll box must still fire, got ${JSON.stringify(out)}`,
    );
  } finally {
    await browser.close();
  }
...
test('confirmOcclusions drops a same-target occlusion whose cover SHIFTED between frames', () => {
...
test('occlusion is SILENT on an on-screen skip-link behind a fixed header (chrome cover)', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    ...
    // overlay.
    await page.setContent('<!doctype html><body style="margin:0">' +
      '<header style="position:fixed;top:0;left:0;width:100%;height:60px;background:#fff;z-index:10">Nav</header>' +
      '<a id="skip2" href="#main" style="position:absolute;left:8px;top:8px;width:140px;height:30px">Skip to content</a>' +
      '</body>');
    const out = await page.evaluate(occlusionScan);
    assert.ok(
...
test('occlusion is SILENT on background controls behind an open modal backdrop', async () => {
  ...
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent('<!doctype html><body style="margin:0">' +
      '<button id="bg-btn" style="position:absolute;left:60px;top:120px;width:120px;height:40px">Behind</button>' +
      '<div class="modal-backdrop" style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:100"></div>' +
      '</body>');
    const out = await page.evaluate(occlusionScan);
    assert.ok(
...
test('occlusion is SILENT on a styled checkbox (label covering its visually-hidden input)', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    ...
    await page.setContent('<!doctype html><body style="margin:0">' +
      '<input type="checkbox" id="btn-check" style="position:absolute;left:20px;top:20px;width:40px;height:40px;opacity:.01">' +
      '<label class="btn" for="btn-check" style="position:absolute;left:20px;top:20px;width:120px;height:40px;background:#0d6efd;color:#fff">Bold</label>' +
      '</body>');
    const out = await page.evaluate(occlusionScan);
    assert.ok(
...
test('tabnabbing FIRES only on an explicit rel="opener" cross-origin _blank link', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage();
    await page.goto('https://example.com/', { waitUntil: 'domcontentloaded' }).catch(() => {});
    await page.setContent('<!doctype html><body>' +
      '<a id="vuln" href="https://evil.example.org/" target="_blank" rel="opener">deliberate opener</a>' +
      '<a id="plain" href="https://other.example.org/" target="_blank">plain blank (safe by default)</a>' +
      '<a id="safe" href="https://other.example.org/" target="_blank" rel="noopener">noopener</a>' +
    ...
    assert.strictEqual(tab.length, 1, `exactly one tabnabbing (the rel=opener x-origin), got ${JSON.stringify(out)}`);
    assert.ok(tab[0].target.includes('deliberate opener'), `the rel=opener link, got ${JSON.stringify(tab)}`);
  } finally {
    await browser.close();
  }
});

test('tabnabbing is SILENT on a plain target=_blank (modern browsers imply noopener)', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage();
    await page.goto('https://example.com/', { waitUntil: 'domcontentloaded' }).catch(() => {});
...
test('focus-loss: a fresh click on a never-focused button is NOT a loss; a loss from an already-focused element IS', async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent('<!doctype html><body style="margin:0">' +
    ...
    await page.addScriptTag({ content:
      `window.armRef = ${focusLossArm.toString()}; window.checkRef = ${focusLossCheck.toString()};` });
    ...
    assert.strictEqual(fp, false, 'a click on a never-focused button ending on <body> is a platform artifact, not a loss');
    ...
    assert.strictEqual(tp, true, 'a focused control that loses ITS OWN focus to <body> on activation is a real loss');
    ...
    assert.strictEqual(other, false, 'focus on a different element than the tapped control is not this control losing focus');
  } finally {
    await browser.close();
  }
});
