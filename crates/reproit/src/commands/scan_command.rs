...
use crate::modes::{a2ui, backend_headless, fuzz};
...
async fn run_app_scan(ctx: &Ctx, config_path: Option<&Path>, args: ScanArgs) -> Result<ExitCode> {
    ...
    let freshness = crate::model::map::map_freshness(&loaded.root)?;
...
fn report_freshness(ctx: &Ctx, freshness: &crate::model::map::MapFreshness) {
    match freshness {
