//! Windows desktop runner (UI Automation backend), dispatched as `reproit __uia`
//! by drive.rs. The Windows twin of the macOS `swift macos-ax.swift` and the
//! Linux `reproit __atspi` runners: it drives ANY native Windows app (WinUI, WPF,
//! and Qt / Avalonia / wxWidgets builds, which all publish to UI Automation)
//! through the UIA tree and prints the framework-agnostic marker protocol every
//! backend emits.
//!
...
//! patterns) and REUSES the canonical signature core (crate::signature) directly
//! instead of re-implementing it, so there is exactly one signature oracle in the
//! binary. Localized Name/text NEVER enters the hash; it is kept only as a
//! display-only label list (docs/signature.md).
//!
//! Env (set by drive.rs):
...
use windows::Win32::UI::WindowsAndMessaging::{
    EnumWindows, FindWindowW, GetWindowThreadProcessId, IsHungAppWindow, IsWindowVisible,
    SetForegroundWindow,
};
...
//
// The app declares its own predicates via the reproit-windows SDK
...
// A native Windows app is a separate process reproit launches, so its stderr is
// a clean diagnostic channel: we pipe it, scrape the markers in a reader thread,
// and re-emit each as the CLI wire line `EXPLORE:INVARIANT` keyed on the signature
// the runner is CURRENTLY on, de-duped per state. The SDK's fuzzer-detection gate
// is `REPROIT_UNDER_FUZZER=1`, set on the launched child; absent (production) the
// registry stays inert. Mirrors the AT-SPI backend exactly; CI-gated (cfg(windows)).
...
/// / the reproit-windows port), so a marker carrying the SDK's sig matches the
...
// stringify/template artifact leaked to the screen. First match wins. The
// undefined/null/NaN regexes are whole-word (so "annulled" never matches), but a
// whole-word hit alone is not proof of a leak: the same word occurs in ordinary
// prose (a .NET dialog body "repro demo crash: null inventory record."). A leak
// artifact IS the label ("null", "Price: null"); prose merely mentions the word
// inside a sentence. So each bare-word candidate then goes through a prose guard
// (label_looks_like_prose) before it is reported, the same length+sentence test
// the [object Object] class already used. Templates ({{..}}/${..}) are always
// artifacts and skip the guard.
...
// rendered U+FFFD replacement character in an element's Name/value is broken
// text encoding reaching the screen. U+FFFD is what a decoder emits on
// malformed input, never a glyph an app renders on purpose, so the test is a
// pure substring check with no false positives. UIA exposes no image pixel
// status and no font load status, so tofu is the only broken-asset class with
// UIA ground truth here (the img/font classes stay web-only). Returns a short
// clipped excerpt around the first U+FFFD (the human detail; the stable node
// key is the finding identity), or None when no replacement char is rendered.
...
// windows-rs exposes the control-type ids as `UIA_<X>ControlTypeId` (i32 newtype).
// We match on the raw i32 so the table is a plain lookup. Roles outside the fixed
// vocabulary fall through to `node` at normalize time inside crate::signature.
...
// AutomationId a WinUI/UWP app assigns to the WindowControl that holds its caption
// strip (system menu + Minimize/Maximize/Close). Win32 apps instead nest the same
// affordances under a TitleBarControl (TITLEBAR_CONTROL_TYPE), so the two skips in
// is_titlebar_root cover both shapes.
const TITLEBAR_AUTOMATION_ID: &str = "TitleBar";

// The window-manager caption/system button AutomationIds. These are fixed ids the
// OS / XAML caption generator emits (not localized display names like "Close
// Calculator"), so matching them is language-independent. A Button carrying one is
// window chrome the fuzzer must never tap: pressing it would close, minimize, or
// reparent the app under test.
const CAPTION_BUTTON_AUTOMATION_IDS: &[&str] = &["Close", "Minimize", "Maximize", "Restore"];

// True when this element roots the window-manager caption subtree the fuzzer must
// not enter. Two shapes carry it: a Win32 TitleBarControl (control type 50037,
// holding the system MenuBar + Close), and a WinUI/UWP WindowControl whose
// AutomationId is 'TitleBar' (Calculator: holding plain Buttons id='Close'/
// 'Minimize'/'Maximize'). Skipping the whole subtree keeps every window-manager
// affordance out of the tappable set, so the fuzzer can never destroy the app by
// tapping Close and then mistake the vanished window for a crash.
...
// True when a control is a window caption/system button that must stay out of the
// tappable set even when it is not inside a recognised title-bar subtree (the
// belt-and-suspenders complement to is_titlebar_root): a Button whose AutomationId
// is a documented caption id. Structural id match, never the English name, so no
// localized name list is needed.
...
// The label (display-only, NEVER hashed): UIA Name, else LegacyIAccessible value.
    ...
    while let Some((el, depth)) = stack.pop() {
        ...
        if role == "textfield" {
            if let Some(id) = aid.as_deref().filter(|id| !id.is_empty()) {
                ...
                let purpose = crate::appmap::normalize_input_purpose(
...
// Invoke/Toggle/Select the control, else the LegacyIAccessible default action.
fn press(el: &IUIAutomationElement) -> bool {
    if let Some(p) = get_pattern::<IUIAutomationInvokePattern>(el, UIA_InvokePatternId.0) {
        if unsafe { p.Invoke() }.is_ok() {
            return true;
        }
    }
    if let Some(p) = get_pattern::<IUIAutomationTogglePattern>(el, UIA_TogglePatternId.0) {
        if unsafe { p.Toggle() }.is_ok() {
            return true;
        }
    }
    if let Some(p) =
        get_pattern::<IUIAutomationSelectionItemPattern>(el, UIA_SelectionItemPatternId.0)
    {
        if unsafe { p.Select() }.is_ok() {
            return true;
        }
    }
    if let Some(p) =
...
fn set_text(el: &IUIAutomationElement, value: &str) -> bool {
    if let Some(vp) = get_pattern::<IUIAutomationValuePattern>(el, UIA_ValuePatternId.0) {
        let b = BSTR::from(value);
        if unsafe { vp.SetValue(&b) }.is_ok() {
            return true;
        }
    }
    false
...
// Synthesize an Escape keypress to the foreground window (the "back" verb and the
// scenario `back`).
fn send_escape() {
    let down = INPUT {
...
// ── HANG oracle (IsHungAppWindow) ───────────────────────────────────────────
fn window_hwnd(window: &IUIAutomationElement) -> HWND {
    unsafe {
        window
...
fn maybe_emit_hang(window: &IUIAutomationElement, from_sig: &str, action: &str) {
...
fn window_exists(window: &IUIAutomationElement) -> bool {
    // A vanished window fails the most basic property read; UIA returns an error
    // (element not available) once the HWND is gone.
...
// write a PNG with the `image` crate. Best-effort: any failure just skips the PNG.
...
// bounds (`-offset_x/-offset_y -video_size WxH -i desktop`), window-region only.
// This is deliberately the SAME coordinate space the finding box is measured in:
// BoundingRectangle gives every element AND the window a screen-pixel, top-left
// origin, so a capture of exactly the window rect makes videoW/H == capture px
// and box = element - windowOrigin land 1:1 with no scale or offset. (gdigrab's
// `-i title=` alternative crops to the window's CLIENT area, dropping the title
// bar + borders -- a different origin than the window rect -- so it is only the
// fallback for when the window bounds are unavailable.)
//
// box-overlay still rescales by (actual video px / videoW) as a safety net, so an
// odd-sized window cropped to even dimensions for the H.264 encoder is handled.
//
// Windows has no clean SIGINT for a child, so finalize by writing `q` to
// ffmpeg's stdin (its documented graceful-quit key), then reaping -- this flushes
// the moov atom and closes the .mov exactly as Control-C would.
...
// ── fuzz config + graph helpers (same shape as backends/tui.rs) ─────────────
...
struct Fuzz {
    ...
    clip_sel: Option<String>, // element label to box (the finding's control)
    clip_label: Option<String>, // caption text drawn on the box
    clip_oracle: Option<String>, // oracle id, echoed back on the FINDING:BOXED marker
}
...
fn load_fuzz() -> Fuzz {
    ...
    f.replay = str_array(&j, "replay");
    f.prefix = str_array(&j, "prefix");
    if let Some(clip) = j.get("clip").and_then(|v| v.as_object()) {
        let sf = |k: &str| clip.get(k).and_then(|v| v.as_str()).map(String::from);
        f.clip_sel = sf("sel");
        f.clip_label = sf("label");
        f.clip_oracle = sf("oracle");
    }
...
fn run_scenario_actor(
    ...
    for _ in 0..100_000u32 {
        ...
        if let Some(name) = act.strip_prefix("shoot:") {
            ...
            emit(&format!(
                "JOURNEY[a] step: auth-restore unsupported on desktop-uia runner; \
...
// Find the first visible top-level HWND owned by `pid`, retried until `timeout`.
// EnumWindows is used instead of walking UIA's desktop root: a broken provider
// elsewhere on the desktop can block FindAll and must not hang attachment to the
// process Reproit itself just launched.
fn window_for_pid(
    automation: &IUIAutomation,
    pid: u32,
    timeout: Duration,
...
pub fn run() -> Result<()> {
    ...
    // top-left origin, so the box is element - windowOrigin), write box-spec.json in
    // the window's own pixel space, then quit ffmpeg so it flushes clip.mov. The
    // host runs box-overlay.mjs (clip.mov + box-spec.json -> boxed clip), the uniform
    // path for every runner that cannot inject a live overlay.
    if clip_armed {
...
// tiny flat parser (a `value_nodes:` block of `- selector` items), so no YAML dep
// is pulled; a missing file/block yields an empty list.
...
// NOTE: this module is cfg(windows)-gated in main.rs, so these tests run only
// on a Windows host/CI. They pin the pure text scan; the COM-facing walk is
// exercised live by the operability-golden Windows job.
#[cfg(test)]
mod tests {
    use super::*;
