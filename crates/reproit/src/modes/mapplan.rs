//! `reproit map semantic`: derive the candidate (hypothesized) map from app source
...
/// `reproit map coverage`: reconcile the existing candidate map against the
        ...
        if json {
            println!(
                ...
                serde_json::json!({
                    ...
                    "error": "no candidate map; run `reproit map semantic` first",
                })
            );
        } else {
            eprintln!("  no candidate map yet; run `reproit map semantic` first");
        }
        return Ok(());
    };
    let verified = map::load_map(&loaded.root, &loaded.config);
...
/// `reproit map converge`: run the loop against the verified map (the passive
