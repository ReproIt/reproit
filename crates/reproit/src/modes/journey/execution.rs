...
pub async fn fuzz_multi_checkpoint(
    ...
    let mut transitions: Vec<(String, String, String)> = map
        ...
        .filter(|t| {
            !matches!(
                t.reversibility,
                crate::model::appmap::Reversibility::Destructive
                    | crate::model::appmap::Reversibility::VerifiedIrreversible
                    | crate::model::appmap::Reversibility::ProposedIrreversible
            )
        })
        .filter(|t| !matches!(t.action, crate::model::appmap::Action::Type { .. }))
    ...
    for run in 0..runs.max(1) {
        ...
        let observed = crate::modes::fuzz::finding_signatures_for_log(&loaded.config, &log);
        ...
        for signature in observed {
            ...
            if confirm {
                ...
                if !done
                    || !crate::modes::fuzz::finding_signatures_for_log(&loaded.config, &confirm_log)
                        .contains(&signature)
                {
                    continue;
                }
                ...
                while i < minimal.len() {
                    ...
                    let (candidate_log, done, _) =
                        run_tagged_scenario_once(loaded, &actors, &checkpoint, &candidate, None)
                            .await?;
                    if done
                        && crate::modes::fuzz::finding_signatures_for_log(
                            &loaded.config,
                            &candidate_log,
                        )
                        .contains(&signature)
            ...
            let (capture_log, captured, run_dir) =
                run_tagged_scenario_once(loaded, &actors, &checkpoint, &minimal, None).await?;
            if !captured
                || !crate::modes::fuzz::finding_signatures_for_log(&loaded.config, &capture_log)
                    .contains(&signature)
            {
                continue;
            }
...
pub(super) fn journey_contract_violations(
    ...
    log: &str,
    actors: &[String],
) -> Vec<crate::model::contracts::ContractViolation> {
    let observations = crate::model::observation::from_runner_log(log, actors);
    crate::model::contracts::evaluate_all(&journey.contracts, &observations)
}
...
pub(super) async fn run_scenario(
    ...
    if !crate::backends::platform::speaks_barrier(platform) {
        bail!(
    ...
    for i in 1..=total {
        ...
        let conductor = crate::modes::barrier::Conductor::start(script.clone(), n).await?;
        ...
        if !completed && !quiet {
            ...
            match stall {
                crate::modes::barrier::Stage::AwaitingJoin(missing) => {
                    let names: Vec<String> = missing
                        .iter()
                        .map(|&i| format!("{} ({})", crate::modes::barrier::letter(i), who(i)))
                        .collect();
                    println!(
                ...
                crate::modes::barrier::Stage::Stalled { dev, action, secs } => {
                    println!(
                        "  scenario stalled: actor {} ({}) never completed `{action}` after \
                         {secs}s",
                        crate::modes::barrier::letter(dev),
                        who(dev),
                    );
                }
                _ => {}
...
pub(super) async fn run_tagged_scenario_once(
    ...
    let script: Vec<(usize, String)> = checkpoint
        ...
        .collect::<Result<_>>()?;
    let conductor = crate::modes::barrier::Conductor::start(script, actors.len()).await?;
...
pub(super) async fn multi_capsule_reproduces(
    ...
    Ok(done
        && crate::modes::fuzz::finding_signatures_for_log(&loaded.config, &log).contains(signature))
}
