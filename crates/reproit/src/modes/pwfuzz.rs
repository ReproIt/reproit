...
//! Playwright test, run it to reach its deep state, then fuzz OUTWARD from
//! there with reproit's oracles. The pitch: "you wrote the test; reproit finds
//! the bugs you didn't" -- in the user's own language, with zero new DSL to
//! learn.
//!
...
//! helper, turns the mapped actions into a `from_prefix` (the per-seed prefix
//! the web runner replays before exploring), pulls the start URL from the
//! test's first `page.goto`, prints an honest capture summary (mapped vs
//! skipped), and hands off to the EXISTING fuzz pipeline.
//!
...
pub struct Capture {
    ...
    /// onward regardless, since a failing test can still leave a usable
    /// prefix).
    #[serde(default)]
    pub passed: bool,
}
...
/// Run the capture helper against `test_path`, using `runner_dir` (the
/// provisioned web runner: playwright + browsers + `pw-capture.mjs`). Returns
/// the parsed `Capture`. The helper exits 0 even on a failing test (a failing
/// test can still leave a usable trace prefix), so a non-zero status here is a
/// real harness error.
pub fn capture(runner_dir: &Path, test_path: &Path, log: &dyn Fn(&str)) -> Result<Capture> {
    let script = runner_dir.join("pw-capture.mjs");
    if !script.exists() {
        bail!(
            "pw-capture.mjs not found in the web runner dir ({}). Re-provision the runner (a \
             `reproit fuzz <url>` run installs it).",
            runner_dir.display()
        );
    }
...
/// of the test's actions reproit replayed, how many it skipped and why, plus
/// the start URL and the auth note. `say` routes lines (stdout, or stderr under
/// --json).
pub fn report(cap: &Capture, test_path: &Path, say: &dyn Fn(&str)) {
    ...
    say(&format!(
        "  replayed {mapped}/{total} action(s) from the test ({} skipped, {pct}% mapped), then \
         fuzzed from there",
        cap.unsupported.len()
    ));
    ...
    if cap.unsupported.is_empty() {
        ...
        say(
            "  fix: give these elements a stable data-testid / id / name, or address them by \
             getByRole(name)/getByText so reproit can replay them.",
        );
    }
    ...
    {
        say(
            "  auth: replaying the test's own fill/click actions signs in inside reproit's runner \
             (no storageState transplant; the typed values are your test's data).",
        );
    }
}
...
mod tests {
    ...
    fn capture_json_converts_to_from_prefix_in_order() {
        let cap = cap_from(
            r#"{
              ...
              "actions": [
                {
                  "kind":"type", "finder":"key:testid:username", "value":"ada",
                  "action":"type:key:testid:username=ada",
                  "raw":"internal:testid=[data-testid=\"username\"s]"
                },
                {
                  "kind":"type", "finder":"key:testid:password", "value":"secret123",
                  "action":"type:key:testid:password=secret123", "raw":"x"
                },
                {
                  "kind":"tap", "finder":"key:testid:continue",
                  "action":"tap:key:testid:continue", "raw":"x"
                }
              ],
    ...
    fn empty_capture_reports_full_coverage() {
        let cap = cap_from(concat!(
            r#"{"baseURL":null,"gotoUrl":null,"actions":[],"#,
            r#""unsupported":[],"notes":[],"passed":false}"#
        ));
