...
//! be performed, so the app diverged from the map). `debug map verify` reuses the
        ...
        match (&step.do_action, &step.goto, &step.expect, &step.fill) {
            ...
            (None, Some(target), None, None) => {
                let m = map.ok_or_else(|| {
                    anyhow::anyhow!(
                        "step {n}: `goto: {target}` needs an app model; run `reproit scan` once to learn the app"
                    )
                })?;
            ...
            (None, None, Some(e), None) => {
                ...
                if let Some(state) = &e.state {
                    let m = map.ok_or_else(|| {
                        anyhow::anyhow!(
                            "step {n}: `expect: state` needs an app model; run `reproit scan` once to learn the app"
                        )
                    })?;
        ...
        match kind {
            SetupKind::Login => {
                ...
                if !exists(root, login_name) {
                    bail!(
                        "`setup: login({acct})` needs `{account_login}` or `login`; run `reproit auth discover {acct}`"
                    );
                }
...
pub async fn fuzz_multi_checkpoint(
    ...
    let mut transitions: Vec<(String, String, String)> = map
        ...
        .filter(|t| {
            !matches!(
                t.reversibility,
                crate::appmap::Reversibility::Destructive
                    | crate::appmap::Reversibility::VerifiedIrreversible
                    | crate::appmap::Reversibility::ProposedIrreversible
            )
        })
        .filter(|t| !matches!(t.action, crate::appmap::Action::Type { .. }))
    ...
    for run in 0..runs.max(1) {
        ...
        let observed = crate::fuzz::finding_signatures_for_log(&loaded.config, &log);
        ...
        for signature in observed {
            ...
            if confirm {
                ...
                if !done
                    || !crate::fuzz::finding_signatures_for_log(&loaded.config, &confirm_log)
                        .contains(&signature)
                {
                    continue;
                }
                ...
                while i < minimal.len() {
                    ...
                    let (candidate_log, done, _) =
                        run_tagged_scenario_once(loaded, &actors, &checkpoint, &candidate, None)
                            .await?;
                    if done
                        && crate::fuzz::finding_signatures_for_log(&loaded.config, &candidate_log)
                            .contains(&signature)
                    {
                        minimal = candidate;
                    } else {
            ...
            let (capture_log, captured, run_dir) =
                run_tagged_scenario_once(loaded, &actors, &checkpoint, &minimal, None).await?;
            if !captured
                || !crate::fuzz::finding_signatures_for_log(&loaded.config, &capture_log)
                    .contains(&signature)
            {
                continue;
            }
...
async fn run_tagged_scenario_once(
    loaded: &config::Loaded,
    actors: &[String],
    ...
    let script: Vec<(usize, String)> = checkpoint
        ...
        .collect::<Result<_>>()?;
    let conductor = crate::barrier::Conductor::start(script, actors.len()).await?;
...
fn build_multi_capsule(
    ...
    let finding = crate::capsule::FindingIdentity {
    ...
    let mut capsule = crate::capsule::Capsule::new(app, finding);
...
async fn multi_capsule_reproduces(
    loaded: &config::Loaded,
    actors: &[String],
    checkpoint: &[(String, String)],
    suffix: &[(String, String)],
    signature: &str,
    candidate: &mut crate::capsule::Capsule,
) -> Result<bool> {
    candidate.finalize_id()?;
    candidate.persist(&loaded.root)?;
    let id = candidate.id.clone();
    let guard = crate::capsule::Capsule::materialize_plaintext(&loaded.root, &id)?;
    let result =
        run_tagged_scenario_once(loaded, actors, checkpoint, suffix, Some(guard.path())).await;
    let _ = std::fs::remove_dir_all(crate::layout::capsule_dir(&loaded.root, &id));
    let (log, done, _) = result?;
    Ok(done && crate::fuzz::finding_signatures_for_log(&loaded.config, &log).contains(signature))
}

async fn shrink_multi_capsule(
    loaded: &config::Loaded,
    actors: &[String],
    checkpoint: &[(String, String)],
    suffix: &[(String, String)],
    ...
    if original_id != best.id {
        let _ = std::fs::remove_dir_all(crate::layout::capsule_dir(&loaded.root, &original_id));
    }
    Ok(best)
}

#[allow(clippy::too_many_arguments)] // Persists the complete multi-actor finding contract.
fn persist_multi_finding(
    loaded: &config::Loaded,
    checkpoint_name: &str,
    seed: u64,
    actors: &[String],
    ...
    let dir = loaded.root.join(".reproit/findings").join(&id);
...
/// app map are used, and the returned journey must still pass a clean run before
/// callers accept it. Multi-screen password/OTP flows are followed through map
/// transitions, with each secret filled when its semantic field appears.
...
mod tests {
    ...
    fn discovers_a_two_screen_phone_otp_login_from_semantics() {
        let map: AppMap = serde_json::from_value(serde_json::json!({
            ...
            "states": {
                "phone": {
                    "description":"Sign in with phone", "signature":{"screenshot_phash":null,"semantics_hash":"a","route":"/login"},
                    "elements":[{"sel":"key:testid:phone","role":"textbox","label":"Número de teléfono","inputPurpose":"phone"}], "texts":[], "parameters":[]
                },
                "verify": {
                    "description":"Verification code", "signature":{"screenshot_phash":null,"semantics_hash":"b","route":"/verify"},
                    "elements":[{"sel":"key:testid:otp","role":"textbox","label":"Код подтверждения","inputPurpose":"otp"}], "texts":[], "parameters":[]
                },
                "home": {
                    "description":"Home", "signature":{"screenshot_phash":null,"semantics_hash":"c","route":"/"},
                    "elements":[], "texts":[], "parameters":[]
                }
            },
            "transitions":[
                {"from":"phone","to":"verify","action":{"kind":"tap","finder":"key:testid:continue"},"guards":[],"reversibility":"verified_irreversible","expected":null},
                {"from":"verify","to":"home","action":{"kind":"tap","finder":"key:testid:verify"},"guards":[],"reversibility":"verified_irreversible","expected":null}
            ]
        })).unwrap();
        let spec = discover_login_spec(
            &map,
            "bob",
            crate::config::AuthStrategy::PhoneOtp,
            Some("Inbox"),
        )
        .unwrap();
