...
//! silently dropped, and a compatibility report (mapped vs unsupported) prints at
//! the end.
...
pub fn run(
    ...
    let lines = translate_all(&commands, &base, 0);
    let yaml = render_journey(&journey_name, app_id.as_deref(), &lines);

    // Compatibility report: count mapped steps and, per unsupported Maestro
    // command, how many times it appeared.
    ...
    // The compatibility report (the trust feature: measurable, honest).
    let total = steps + todo_total;
    let pct = (steps * 100).checked_div(total).unwrap_or(100);
    ctx.say(format!(
        "\nMaestro compatibility: {steps}/{total} commands mapped ({pct}%)."
    ));
    if unsupported.is_empty() {
        ...
        ctx.say(
            "  fixes: give tapped elements stable ids (Maestro text -> reproit \
             tap:key:); replace scroll/swipe with a goto: or a keyed tap:; port \
...
fn translate_all(cmds: &[Value], base: &Path, depth: usize) -> Vec<Line> {
    cmds.iter()
        .flat_map(|c| translate(c, base, depth))
        .collect()
}

/// Translate one Maestro command. Returns multiple lines because `runFlow`
/// inlines a sub-flow and `repeat` unrolls.
fn translate(cmd: &Value, base: &Path, depth: usize) -> Vec<Line> {
    ...
    match cmd {
        ...
        Value::Mapping(m) if m.len() == 1 => {
            ...
            match key {
                ...
                "runFlow" => run_flow(v, base, depth),
                "repeat" => repeat_block(v, base, depth),
...
fn run_flow(v: &Value, base: &Path, depth: usize) -> Vec<Line> {
    if depth >= MAX_DEPTH {
    ...
    if let Value::Mapping(m) = v {
        if let Some(Value::Sequence(cmds)) = m.get(Value::from("commands")) {
            ...
            out.extend(translate_all(cmds, base, depth + 1));
    ...
    out.extend(translate_all(&cmds, &inner_base, depth + 1));
...
fn repeat_block(v: &Value, base: &Path, depth: usize) -> Vec<Line> {
    ...
    if m.contains_key(Value::from("while")) {
        ...
        out.extend(translate_all(&body, base, depth));
        return out;
    }
    let times = m
...
fn render_journey(name: &str, app_id: Option<&str>, lines: &[Line]) -> String {
    ...
    out.push_str("# equivalent and were left as comments (see the compatibility report).\n");
...
mod tests {
    ...
    fn repeat_unrolls_a_literal_count() {
        ...
        assert!(matches!(v[0], Line::Note(_)));
        assert_eq!(v[1], Line::Step("do: tap:label:Next".into()));
        assert_eq!(v[3], Line::Step("do: tap:label:Next".into()));
    }
    ...
    fn run_flow_inlines_inline_commands_and_drops_the_when_condition() {
        ...
        assert!(v
            .iter()
            .any(|l| matches!(l, Line::Step(s) if s == "do: tap:label:Dismiss")));
        assert!(v
            .iter()
    ...
    fn parses_two_doc_flow_and_renders() {
        ...
        let lines = translate_all(&cmds, Path::new("."), 0);
