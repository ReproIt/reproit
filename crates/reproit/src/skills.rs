...
/// Output format. The playbook content is identical; the packaging differs. We
/// target FORMATS, not vendors: `agents` (AGENTS.md) is the Linux Foundation
/// cross-agent standard read by Codex, opencode, Cursor, Amp, Aider, Zed,
/// Copilot, Windsurf, Claude Code, and more, so it covers the field with one
/// file; `skill` is the richer progressive-disclosure format (Claude Code +
/// opencode). `--dir` places either anywhere for the long tail.
#[derive(Clone, Copy, Debug, PartialEq, Eq, ValueEnum)]
pub enum Format {
    /// AGENTS.md: the cross-agent open standard. Broadest reach (the default).
    Agents,
    /// Agent Skills: a SKILL.md tree (Claude Code / opencode), with on-demand
    /// references.
    Skill,
}
...
pub fn install(format: Format, global: bool, dir: Option<PathBuf>) -> Result<()> {
    match format {
        Format::Agents => install_agents(global, dir),
        Format::Skill => install_skill(global, dir),
    }
}
...
mod tests {
    ...
    // satisfy). `Graph` is an internal finding class, not a user-facing oracle.
    #[test]
    ...
    fn skill_core_commands_exist_and_are_documented() {
        const CORE: &[&str] = &[
            "map", "sweep", "fuzz", "check", "keep", "record", "repro", "repros", "watch",
            "secrets", "journey", "cloud",
        ];
