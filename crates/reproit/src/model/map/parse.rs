...
use crate::model::appmap::{OperabilityGap, OperabilityGaps, StateElement, StateText};
    ...
    for event in events {
        ...
        if let Some(json) = extract(line, "EXPLORE:STATE ") {
                ...
                if let Some(els) = json.get("elements").and_then(Value::as_array) {
                    ...
                    let elements: Vec<StateElement> = els
                        ...
                        .filter_map(|e| {
                            ...
                            let input_purpose = crate::model::appmap::normalize_input_purpose(
            ...
            if let (Some(sig), Some(items)) = (
                ...
                let parsed: Vec<RelationViolation> = items
                    .iter()
                    .filter_map(|it| {
                        let kind = it.get("kind").and_then(Value::as_str)?;
                        let dependent_key = it.get("dependentKey").and_then(Value::as_str)?;
                        let owner_key = it.get("ownerKey").and_then(Value::as_str)?;
                        let container_key = it.get("containerKey").and_then(Value::as_str)?;
                        let violation = it.get("violation").and_then(Value::as_str)?;
                        // Unknown future relationship/violation classes abstain
                        // instead of silently entering the confirmed product set.
                        ...
                        let max_gap = it.get("maxGap").and_then(Value::as_i64).unwrap_or(8);
                        if !(0..=64).contains(&max_gap) {
                            return None;
                        }
                        let gap = it.get("gap").and_then(Value::as_f64).unwrap_or(0.0);
                    ...
                    .collect();
                if !parsed.is_empty() {
                    obs.relations.insert(sig.to_string(), parsed);
                }
            }
        } else if let Some(json) = extract(line, "EXPLORE:RELATIONSTATUS ") {
            if let (Some(sig), Some(checks)) = (
                json.get("sig").and_then(Value::as_str),
                json.get("checks").and_then(Value::as_array),
            ) {
                let parsed: Vec<RelationCheck> = checks
                    .iter()
                    .filter_map(|it| {
                        let kind = it.get("kind").and_then(Value::as_str)?;
                        let dependent_key = it.get("dependentKey").and_then(Value::as_str)?;
