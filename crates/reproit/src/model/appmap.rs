...
pub struct StateElement {
    ...
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub label: String,
    /// Locale-independent semantic input purpose from platform metadata.
    #[serde(
        rename = "inputPurpose",
        default,
...
/// Canonicalize platform vocabulary into the cross-backend auth contract.
/// The reserved stable-id convention makes metadata-poor and instrumented
/// surfaces universal without inspecting localized labels:
/// `reproit-purpose-<purpose>--<developer-id>`.
pub fn normalize_input_purpose(raw: Option<&str>, selector: &str) -> Option<String> {
    let from_selector = selector
        .split("reproit-purpose-")
        .nth(1)
        .and_then(|tail| tail.split("--").next());
    let value = raw.or(from_selector)?.trim().to_ascii_lowercase();
    let canonical = match value.replace(['_', ' '], "-").as_str() {
        "username" | "user-name" => "username",
        "email" | "email-address" => "email",
        "phone" | "tel" | "telephone" | "telephone-number" | "telephonenumber" => "phone",
        "password" | "current-password" => "password",
        "new-password" => "new-password",
        "otp" | "one-time-code" | "onetimecode" | "sms-otp" => "otp",
        "passkey" | "webauthn" => "passkey",
        "recovery-code" | "backup-code" => "recovery-code",
        _ => return None,
    };
...
mod auth_contract_tests {
    use super::normalize_input_purpose;

    #[test]
    fn purpose_contract_normalizes_platform_terms_and_structural_annotations() {
        assert_eq!(
            normalize_input_purpose(Some("one-time-code"), "key:x").as_deref(),
            Some("otp")
        );
        assert_eq!(
            normalize_input_purpose(Some("telephoneNumber"), "key:x").as_deref(),
            Some("phone")
        );
        assert_eq!(
            normalize_input_purpose(None, "key:reproit-purpose-email--login-id").as_deref(),
            Some("email")
        );
        assert_eq!(normalize_input_purpose(None, "key:correo"), None);
    }
}
