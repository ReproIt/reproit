...
pub(super) fn load_existing_map_unlocked(root: &Path) -> Result<Option<AppMap>> {
    ...
    if matches!(map.schema_version, 1 | 2) {
        map.schema_version = crate::domain::appmap::APP_MAP_SCHEMA_VERSION;
    }
...
fn validate_map(map: &AppMap, path: &Path) -> Result<()> {
    if map.schema_version != crate::domain::appmap::APP_MAP_SCHEMA_VERSION {
        anyhow::bail!(
            "{} uses unsupported app-map schema {} (this build supports {})",
            path.display(),
            map.schema_version,
            crate::domain::appmap::APP_MAP_SCHEMA_VERSION
        );
    }
