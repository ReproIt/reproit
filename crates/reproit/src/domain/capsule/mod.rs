...
mod redaction;
mod retention;
use retention::maybe_rotate_key;
pub use retention::prune_unreferenced;
#[cfg(test)]
pub use retention::rotate_key;
mod typed;
use crypto::{
...
impl Capsule {
    ...
    pub fn persist(&mut self, root: &Path) -> Result<std::path::PathBuf> {
        ...
        let dir = crate::runtime::project_layout::capsule_dir(root, &id);
        ...
        let (max_count, max_age) = retention::bounds();
        let _ = prune_unreferenced(root, Some(&id), max_count, max_age);
        Ok(dir)
    }

    /// Whether this capsule's ciphertext is present in the local store.
    ///
    /// The store is machine-local and gitignored, so absence is an ordinary
    /// state (fresh clone, reset, guard recorded elsewhere), not a corruption.
    /// Callers that REQUIRE a capsule use this to report an honest stale
    /// verdict instead of failing with an I/O error.
    pub fn exists(root: &Path, id: &str) -> bool {
        crate::runtime::project_layout::capsules_dir(root)
            .join(id)
            .is_dir()
    }
    ...
    pub fn materialize_plaintext(root: &Path, id: &str) -> Result<PlaintextGuard> {
        ...
        let dir = crate::runtime::project_layout::tmp_dir(root).join("capsules");
        std::fs::create_dir_all(&dir)?;
        ...
        for entry in std::fs::read_dir(run_dir)? {
            ...
            let parsed = if name.starts_with("backend-") && name.ends_with(".jsonl") {
                ...
                for (line_no, line) in raw.lines().enumerate() {
                    ...
                    parsed.push(
                        serde_json::from_str::<crate::domain::backend::BackendEvent>(line)
                            .with_context(|| {
                ...
                crate::domain::backend::parse_events(&std::fs::read_to_string(&path)?)
            } else {
                Vec::new()
            };
            for event in parsed {
