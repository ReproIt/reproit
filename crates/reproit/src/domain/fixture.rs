...
/// The deterministic value generator. Honors `empty` (wins outright), the v1
/// directives (`charset`, `emoji`, `rtl`, `minLen`) and the richer v2
/// directives cloud emits from PII-safe fingerprints (`minBytes`,
/// `minGraphemes`, `scripts`, `combining`, `zeroWidth`, `newline`,
/// `edgeWhitespace`). Values are features-matching, never user data.
fn generate_value(g: &Map<String, Value>) -> String {
    ...
    let min_bytes = g.get("minBytes").and_then(|v| v.as_u64()).unwrap_or(0) as usize;
    let min_graphemes = g.get("minGraphemes").and_then(|v| v.as_u64()).unwrap_or(0) as usize;
    ...
    if let Some(scripts) = g.get("scripts").and_then(|v| v.as_array()) {
        for script in scripts.iter().filter_map(|v| v.as_str()) {
            if let Some(sample) = script_sample(script) {
                out.push_str(sample);
            }
        }
    }
    ...
    if g.get("newline").and_then(|v| v.as_bool()).unwrap_or(false) {
        out.push('\n');
    }

    // Always lay down at least one base run so charset is represented even when
    // minLen/minBytes are 0 or already covered by requested feature sentinels.
    let target_chars = min_len.max(out.chars().count() + base.len());
    let mut i = 0;
    let mut filler_graphemes = 0;
    while out.chars().count() < target_chars
        || out.len() < min_bytes
        || filler_graphemes < min_graphemes
    {
...
fn script_sample(script: &str) -> Option<&'static str> {
    match script {
        "Latin" => Some("Latin"),
        "Arabic" => Some("عربي"),
        "Hebrew" => Some("עברית"),
        "Cyrillic" => Some("Кириллица"),
        "Greek" => Some("Ελληνικά"),
        "Han" | "Hani" => Some("漢字"),
        "Hiragana" => Some("ひらがな"),
        "Katakana" => Some("カタカナ"),
        "Hangul" => Some("한글"),
        "Devanagari" => Some("देवनागरी"),
        "Thai" => Some("ไทย"),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn v2_directives_are_synthesized_into_matching_features() {
        let spec: Value = serde_json::from_str(include_str!(
            "../../tests/golden/fixtures/fixture-spec-v2.json"
        ))
        .unwrap();
        let f = synthesize(&spec);
        let value = &f.fields[0].value;

        assert_eq!(f.locale.as_deref(), Some("tr"));
        assert_eq!(f.fields[0].field, "name");
        assert!(
            value.chars().count() >= 24,
            "minGraphemes target gets enough base graphemes"
        );
        assert!(value.chars().count() >= 18, "minLen honored");
        assert!(value.len() >= 90, "minBytes honored");
        assert!(value.contains('🚀'), "emoji honored");
        assert!(value.contains("Latin"), "Latin script sample honored");
        assert!(value.contains("عربي"), "Arabic script sample honored");
        assert!(value.contains('\u{0301}'), "combining mark honored");
        assert!(value.contains('\u{200B}'), "zero-width marker honored");
        assert!(value.contains('\n'), "newline honored");
        assert!(value.starts_with(" \t"), "leading edge whitespace honored");
        assert!(value.ends_with("\t "), "trailing edge whitespace honored");
        assert!(value
            .chars()
            .any(|c| ('\u{0590}'..='\u{06FF}').contains(&c)));
    }
    ...
    #[test]
    fn v2_contract_fixture_is_deterministic() {
        let spec: Value = serde_json::from_str(include_str!(
            "../../tests/golden/fixtures/fixture-spec-v2.json"
        ))
        .unwrap();
    ...
    #[test]
    fn v2_contract_fixture_matches_cloud_mirror_when_checked_out() {
        let local = include_str!("../../tests/golden/fixtures/fixture-spec-v2.json");
        let sibling =
            std::path::Path::new("../reproit-cloud/tests/golden/fixtures/fixture-spec-v2.json");
        if sibling.exists() {
