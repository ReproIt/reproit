...
pub(crate) fn from_runner_events(
    events: &[crate::domain::runner::RunnerEvent<'_>],
    actor_names: &[String],
) -> Vec<Observation> {
    let mut out = Vec::new();
    let mut actor = None;
    ...
    let has_full_observations = events.iter().any(|event| {
        matches!(
            event,
            crate::domain::runner::RunnerEvent::Fuzz(line) if line.starts_with("FUZZ:OBS ")
        )
    });

    for event in events {
        let line = match *event {
            crate::domain::runner::RunnerEvent::Fuzz(line) => line,
            crate::domain::runner::RunnerEvent::Explore(line) => {
                if has_full_observations && line.starts_with("EXPLORE:STATE ") {
                    continue;
                }
                line
            }
            crate::domain::runner::RunnerEvent::Backend(_) => continue,
        };
