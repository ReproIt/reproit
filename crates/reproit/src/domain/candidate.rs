...
impl Candidate {
    ...
    fn matches(&self, name: &str, state: &crate::domain::appmap::State) -> bool {
    ...
    pub fn find_in<'a>(&self, map: &'a AppMap) -> Option<&'a crate::domain::appmap::State> {
        map.states
            .iter()
...
mod tests {
    ...
    fn save_writes_candidate_map_to_documented_layout() {
        ...
        assert!(
            crate::runtime::project_layout::candidate_map_path(&root).exists(),
