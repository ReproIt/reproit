...
use crate::adapters::drive::{spawn_drive, Drive, RunCtx};
use crate::adapters::reset::run_reset;
...
use watch::{memory_summary, newest_runner_video, wait_watching, DriveWatch, PhaseTimer};
...
struct DeviceManifest {
    ...
    frames: Option<crate::adapters::frames::FrameSummary>,
...
pub async fn run_journey(
    ...
    let plat = crate::adapters::platform::resolve(&cfg.app.platform)
    ...
    if plat.backend == crate::adapters::platform::Backend::FlutterDrive {
        ...
        crate::adapters::project_scaffold::ensure_integration_test_dep(&project_dir)?;
        ...
        if journey == "explore" {
            ...
            crate::adapters::project_scaffold::vendor_sim_explorer(
                &project_dir,
                &jd,
                &cfg.journeys.driver,
            )?;
    ...
    if byo_target {
        let tag = match plat.backend {
            crate::adapters::platform::Backend::Appium => "rn",
            crate::adapters::platform::Backend::WebCdp => "web",
            _ => "app",
        };
    ...
    let secrets = match crate::adapters::credentials::secret_env(&cfg.auth, root) {
        Ok(s) => s,
        Err(e) => {
    ...
    let target = if plat.backend == crate::adapters::platform::Backend::FlutterDrive {
        if journeys_dir
    ...
    {
        let states: Vec<(String, Arc<Mutex<crate::adapters::drive::DriveState>>)> = drives
            .iter()
        ...
        tokio::spawn(async move {
            ...
            while !stop.load(Ordering::Relaxed) {
                for (label, state) in &states {
                    ...
                    if let Ok(sample) = crate::adapters::vmservice::sample_memory(&url).await {
    ...
    for drive in &drives {
        ...
        let Some(url) = url else { continue };
        if let Ok(snapshot) = crate::adapters::vmservice::collect_coverage(&url).await {
    ...
    let device_manifests: Vec<DeviceManifest> = sims
        ...
        .map(|(i, (sim, drive))| DeviceManifest {
            ...
            frames: std::fs::read_to_string(&drive.log_path)
                .ok()
                .and_then(|log| crate::adapters::frames::process(&run_dir, &drive.label, &log)),
...
pub fn headless_tier_available(cfg: &Config) -> bool {
    crate::adapters::platform::resolve(&cfg.app.platform)
        .map(|p| p.backend == crate::adapters::platform::Backend::FlutterDrive)
        .unwrap_or(false)
}
...
pub async fn run_journey_headless(
    ...
    let log = crate::adapters::credentials::redact(&log, &secrets);
