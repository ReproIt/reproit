...
mod tests {
    ...
    fn resolver_with_element_and_text(
        element_label: &str,
        sel: &str,
        texts: Vec<crate::domain::appmap::StateText>,
    ) -> TapResolver {
        let mut states = BTreeMap::new();
        states.insert(
            "home".to_string(),
            crate::domain::appmap::State {
                name: None,
                description: "home".to_string(),
                signature: crate::domain::appmap::StateSignature {
                    screenshot_phash: None,
                    semantics_hash: Some("sig-home".to_string()),
                    route: None,
                },
                elements: vec![crate::domain::appmap::StateElement {
                    sel: sel.to_string(),
        ...
        TapResolver::from_map(&AppMap {
            app: "demo".to_string(),
            schema_version: crate::domain::appmap::APP_MAP_SCHEMA_VERSION,
            revision: 1,
            states,
            transitions: vec![],
            invariants: vec![],
    ...
    fn text_taps_resolve_through_text_bounds_when_label_is_missing() {
        let resolver = resolver_with_element_and_text(
            "",
            "role:button#0",
            vec![crate::domain::appmap::StateText {
