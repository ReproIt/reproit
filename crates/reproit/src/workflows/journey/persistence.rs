...
pub fn discover_login_spec(
    map: &AppMap,
    account: &str,
    strategy: crate::adapters::config::AuthStrategy,
    validate_text: Option<&str>,
) -> Result<String> {
    use crate::domain::appmap::Action;
    let start = map
        .states
        .iter()
    ...
    let required: Vec<&str> = match strategy {
        crate::adapters::config::AuthStrategy::Password => vec!["identifier", "password"],
        crate::adapters::config::AuthStrategy::PasswordOtp => {
            vec!["identifier", "password", "totp"]
        }
        crate::adapters::config::AuthStrategy::PhoneOtp => vec!["phone", "otp"],
        crate::adapters::config::AuthStrategy::EmailLink => vec!["email"],
