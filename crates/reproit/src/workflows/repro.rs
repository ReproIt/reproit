...
/// Find the latest fuzz finding artifact: the most-recent dir that holds a
/// `fuzz.md` (the discovered repro report). Findings live in TWO stores: UI
/// fuzz run dirs under the evidence out dir, and persisted backend findings
/// under the findings store. Bare `reproit keep` means "the newest finding",
/// whichever store it landed in.
pub(super) fn latest_finding(loaded: &config::Loaded) -> Option<Finding> {
    ...
    for base in [
        ...
        layout::findings_dir(&loaded.root),
    ] {
        ...
        for e in entries.flatten() {
            ...
            if p.is_dir() && p.join("fuzz.md").exists() {
                let t = std::fs::metadata(p.join("fuzz.md"))
...
pub(super) async fn check_repro(
    ...
    let dir = repro::repro_dir(&loaded.root, id);
    let frozen_contract = crate::domain::contracts::FrozenContractGuard::load(
        &dir.join("contract.json"),
    )
    .or_else(|| {
        crate::domain::contracts::FrozenContractGuard::load(
            &layout::finding_dir(&loaded.root, id).join("contract.json"),
        )
    });
    let frozen_backend =
        crate::domain::backend::FrozenBackendGuard::load(&dir.join("backend-contract.json"))
            .or_else(|| {
                crate::domain::backend::FrozenBackendGuard::load(
    ...
    for _ in 0..drive_count {
        ...
        for seg in segments {
            ...
            if let Some(guard) = &frozen_contract {
                let parsed = crate::domain::runner::ParsedRun::new(seg, &[], true, false);
