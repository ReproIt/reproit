...
pub(crate) fn finding_signatures_for_log(cfg: &Config, log: &str) -> BTreeSet<String> {
    ...
    let (confirmed, _) = crate::domain::oracle::OracleFilter::stable().apply(findings);
    confirmed
        .iter()
        .filter(|finding| {
            !finding
...
fn invariant_observations(
    mut obs: crate::domain::map::RunObs,
    exceptions: Vec<Value>,
    sim: bool,
    escapable_route_labels: crate::domain::map::EscapableRoutes,
) -> crate::domain::invariants::Observations {
...
pub(super) fn findings_from_log(
    ...
    log: &str,
    sim: bool,
    escapable: crate::domain::map::EscapableRoutes,
) -> Vec<Value> {
...
pub(super) struct NormalizedEvidence<'a> {
    pub(super) observations: &'a [crate::domain::observation::Observation],
    pub(super) backend_events: &'a [crate::domain::backend::BackendEvent],
    pub(super) stream_defects: &'a [crate::domain::runner::StreamDefect],
}

pub(super) fn findings_from_parsed(
    cfg: &Config,
    obs: crate::domain::map::RunObs,
    exceptions: Vec<Value>,
    sim: bool,
    escapable: crate::domain::map::EscapableRoutes,
    ...
    let mut f = crate::domain::invariants::evaluate(&inv_obs, &cfg.invariants);
    if !cfg.invariants.no_exception {
        f.extend(exceptions);
    }
    if !cfg.contracts.is_empty() {
        f.extend(
            crate::domain::contracts::evaluate_stream(
                &cfg.contracts,
            ...
            .map(crate::domain::contracts::finding),
        );
    }
    if cfg.backend.enabled {
        f.extend(
            crate::domain::backend::evaluate(&cfg.backend, evidence.backend_events)
                .iter()
                .map(crate::domain::backend::finding),
        );
    }
    f
}
...
pub(super) fn findings_for_tier(cfg: &Config, run_dir: &Path, sim: bool) -> Vec<Value> {
    ...
    let backend_violations = crate::domain::backend::evaluate(&cfg.backend, &parsed.backend);
    let _ = crate::domain::backend::write_evidence(
