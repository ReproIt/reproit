...
/// Invoke one gRPC operation in process. The heavy lifting lives in the `grpc`
/// module: it resolves the method from the descriptor pool and drives the
/// HTTP/2 call with `tonic`. This keeps `invoke_traced` transport-agnostic.
pub(super) async fn invoke_grpc(artifact: &RequestArtifact) -> Result<Value> {
    super::grpc::invoke(artifact).await
}
