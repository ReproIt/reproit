...
pub(super) async fn auth_cmd(
    ...
    match action {
        AuthAction::Add {
            ...
            let mut vault = credentials::Vault::open(&vpath)?;
            ...
            if let Some(secret) = totp_secret {
                let Some(code) = credentials::totp_now(&secret) else {
        ...
        AuthAction::Set { key, value } => {
            ...
            let mut v = credentials::Vault::open(&vpath)?;
            v.set(&key, &val);
            v.save()?;
        ...
        AuthAction::List => {
            let v = credentials::Vault::open(&vpath)?;
        ...
        AuthAction::Test { account } => {
            ...
            let env = credentials::secret_env(&loaded.config.auth, &loaded.root)?;
            let ns = format!(
                "REPROIT_SECRET_{}",
                account
                    .chars()
