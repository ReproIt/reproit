...
fn read_series(run_dir: &Path, field: &str) -> Vec<(u64, u64)> {
    let parse_line = |l: &str| -> Option<(u64, u64)> {
        ...
        Some((v.get("t_ms")?.as_u64()?, v.get(field)?.as_u64()?))
    };
...
pub async fn soak(cfg: &Config, root: &Path, args: &SoakArgs) -> Result<bool> {
    ...
    let byte_leak = per_cycle > LEAK_BYTES_PER_CYCLE;

    // DETERMINISTIC signal: live DOM-element growth per cycle (machine-invariant),
    // from the `dom_nodes` field of the same samples. Empty for runners that don't
    // report it (Flutter VM tier), leaving the byte verdict untouched there.
    let dom_series = read_series(&outcome.run_dir, "dom_nodes");
    let dom_per_cycle = if dom_series.len() >= 2 {
        least_squares_slope(&dom_series) * (dom_series.len() - 1) as f64 / args.repeats as f64
    } else {
        0.0
    };
    let dom_leak = dom_series.len() >= 2 && dom_per_cycle >= LEAK_DOM_NODES_PER_CYCLE;
    ...
    let exceptions = crate::workflows::fuzz::app_exceptions(&outcome.run_dir).len();
    let verdict = if dom_leak {
        "LEAK"
    } else if byte_leak {
        "LEAK (advisory: heap-bytes only, no DOM growth)"
    } else {
        "resource-neutral"
    };
    println!(
    ...
    if dom_series.len() >= 2 {
        println!(
            "  dom nodes: {} -> {} ({:+.1} nodes/cycle, deterministic; threshold {:.0})",
            dom_series.first().unwrap().1,
            dom_series.last().unwrap().1,
            dom_per_cycle,
            LEAK_DOM_NODES_PER_CYCLE
        );
    }
    println!(
