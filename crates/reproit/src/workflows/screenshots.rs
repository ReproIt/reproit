...
use crate::adapters::config::Loaded;
use crate::adapters::orchestrator::{self, RunOpts};
use crate::adapters::scoped_env::ScopedEnv;
use crate::domain::target::RunTarget;
...
pub async fn run(ctx: &Ctx, loaded: &Loaded, args: Args) -> Result<bool> {
    ...
    if !crate::workflows::journey::exists(&loaded.root, &tour) {
        bail!(
            "tour journey {:?} not found (expected {})",
            tour,
            crate::workflows::journey::journey_path(&loaded.root, &tour).display()
        );
    }
        ...
        for dev in &device_runs {
            for loc in &locale_runs {
                ...
                let defines: Vec<(String, String)> = match loc {
                    Some(l) => vec![(crate::domain::locale::LOCALE_ENV.to_string(), l.clone())],
                    None => Vec::new(),
                };
