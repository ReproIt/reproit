...
pub struct InvariantsCfg {
    ...
    #[serde(default = "default_true")]
    pub no_leak: bool,
    /// Presented-frame flicker detection. The legacy name is retained for config
    /// compatibility; DOM node replacement alone is diagnostic, not a finding.
    #[serde(default = "default_true")]
    ...
    /// rotates the surface (portrait <-> landscape / split-screen), reflows, then
    /// rotates BACK to the original orientation; a screen that does not rebuild
    /// the same structure lost content/state across the orientation lifecycle.
    /// From the native (Flutter, Appium) and Chromium (web, electron, tauri)
    /// explorers' `EXPLORE:ROTATION`; round-trip identity with value-state
    /// excluded, so it is deterministic and false-positive-free.
    #[serde(default = "default_true")]
    pub no_rotation_loss: bool,
    /// Background-restore-stability: a state must survive the app background ->
    /// foreground lifecycle. The explorer backgrounds the app (paused/hidden) and
    /// restores it (resumed/visible); an app that returns to a different screen
    /// or loses state violates it. From the native (Flutter, Appium) and
    /// Chromium (web, electron, tauri) explorers' `EXPLORE:BGRESTORE`; no size
    /// change and value-state excluded, so it is deterministic and
    /// false-positive-free.
    #[serde(default = "default_true")]
    ...
    /// Evaluation is on by default; the RUNNER probe that drives the revisit loop
    /// is opt-in per run via REPROIT_LISTENERLEAK=1 (repeated navigation changes
    /// exploration semantics, like the duplicate-submit probe). From the
    /// web/electron runners (`EXPLORE:LISTENERLEAK`); sequence/repeat-dependent,
    /// so it belongs to fuzz/soak, not the state-present scan crawl.
    #[serde(default = "default_true")]
    pub no_listener_leak: bool,
    /// Wakelock leak: a wakelock (or a window FLAG_KEEP_SCREEN_ON) acquired on a
    /// screen is still held after the user navigates away, keeping the CPU/screen
    /// awake off the video/map/call screen that needed it (a battery drain). From
    /// the Android/Appium explorer's `EXPLORE:WAKELOCK`, off `dumpsys power`
    /// ground truth compared before vs after leaving the screen; app-global,
    /// released, and short-lived locks are excluded, so it is deterministic and
    /// false-positive-free. Android only (iOS has no public wakelock
    /// introspection; web/desktop/TUI have no wakelock concept).
    #[serde(default = "default_true")]
    ...
    /// (`EXPLORE:SAFEAREA`); deterministic and false-positive-free. Native mobile
    /// only (desktop has no insets; headless web reports every inset as 0).
    #[serde(default = "default_true")]
    pub no_safe_area: bool,
    /// Permission dead-end: under a runtime-permission denial sweep, a screen the
    /// app reached after the denial is a genuine graph dead end (a stuck "please
    /// enable X" screen with no way forward). Uses the internal sink predicate
    /// but attributes the trap to the denied permission.
    /// From the native explorers' `EXPLORE:PERMISSIONWALK` under the denial sweep;
    /// silent outside it. Native mobile only (a backend that cannot deny a
    /// permission never emits the marker).
    #[serde(default = "default_true")]
...
pub struct App {
    ...
    /// are in-process Rust subcommands (`reproit __uia` / `__atspi`) and need no
    /// script here. Default: REPROIT_RUNNERS env, else a `runners/` dir beside
    /// the config.
...
/// Fetch a small text asset (the checksum file) over curl. Returns `Ok(None)` only
/// when the asset is absent (HTTP 404), so network errors and server failures do
/// not silently downgrade checksum verification.
